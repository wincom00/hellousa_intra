# 헬로USA 인트라넷 (HelloUSA Intranet ERP) - Claude Code 가이드
...
> 참고: 상위 폴더 `d:\www\CLAUDE.md`는 **다른 프로젝트(파란여행사 paran_erp)** 용 가이드다. 본 문서(`hellousa_intra`)와 혼동하지 말 것. 두 시스템은 코드 계보를 공유하지만 DB·도메인·인증 쿠키가 다르다.
