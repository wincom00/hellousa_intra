		...
		if ($estimateCode == "") {
			  ...
			  $rst5 = $dbConn->query($qry5);
			   ...
			   if ($tourcomp !="") {
						...
						$qryr1 = "select part_id from $rand_company where reserveCode = '$estimateCode' && money_type='credit' && p_memo !='발권'";
						...
						if ($rowr1[part_id] != $tourcomp) {
							...
							$qryc = "delete from $rand_company where part_id ='$tourcomp' && reserveCode = '$estimateCode' && money_type='credit' && p_memo !='발권'";
               ...
               if ($tourcomp1) {
						...
						$qryr1 = "select part_id from $rand_company where reserveCode = '$estimateCode' && money_type='debit' && p_memo !='발권'";
				        $rstr1 = $dbConn->query($qryr1);
						$rowr1 = $rstr1->fetch_assoc();
						...
						if ($rowr1[part_id] != $tourcomp1) {
							...
							$qry4 ="
											update $rand_company 
												...
												rand_date ='$startDate'
												where
												reserveCode = '$estimateCode' && part_id='$tourcomp1' && money_type = 'debit' && p_memo !='발권'";
			   ...
			   for($k=0; $k<count($pnrnum); $k++)
			   {
					...
					if($pnrnum[$k])
					{
						...
						// 넣기전에 이미 있는지 체크한다.
						$pre_qry1 = "select max(seq_no) as seq from $rand_company where  part_id='$rand_id_air[$k]' && reserveCode = '$estimateCode' && p_memo='발권'
