<?php
	...
	*/
	...
	if ($mode=='edit') {
		  ...
		  //echo $seq;
			if ($chkvalue == 1) {
			   $qry1 = "update dong_mlist2 set chk_sub = '1' where seq_no = '$seq'";
		  } else {
		  	   $qry1 = "update dong_mlist2 set chk_sub = '0' where seq_no = '$seq'";
		  }
		  //echo $qry1;
	  ...
	  echo "<script> alert('Complete!'); </script>";
		  ...
		  //echo $seq;
			if ($chkvalue == 1) {
			   $qry1 = "update dong_mlist2 set chk_send = '1' where seq_no = '$seq'";
		  } else {
		  	   $qry1 = "update dong_mlist2 set chk_send = '0' where seq_no = '$seq'";
		  }
		  //echo $qry1;
		   $rst1 = $dbConn->query($qry1);
	  ...
	  echo "<script> alert('Complete!'); </script>";
		 ...
		 $qry1 = "update dong_mlist2 set chk_send = '0' ";
		  $rst1 = $dbConn->query($qry1);
		 echo "<script> alert('Complete!'); </script>";
	
	} else if ($mode == "del") {
		
		$qry1 = "delete from dong_mlist2  where seq_no = '$seq' ";
		
		$rst1 = $dbConn->query($qry1);
		echo "<script> alert('Deleted!'); </script>";
	} else if ($mode == "add") {
		  $qry1 ="
							insert into dong_mlist2 
								( 
								m_name, 
								mail_addr, 
								chk_sub,
	  ...
	  $qry1 = "update dong_mlist2 set m_name = '$cname',mail_addr='$emailad',area='$country' ,tel_num='$phonenum' where seq_no = '$seq'";
	  $rst1 = $dbConn->query($qry1);
	...
	function printContentlist() {
		...
		$qry1 = "SELECT * FROM dong_mlist2 AS a WHERE 1=1 $name_qry $name_qry1 order by wdate desc";
		///echo $qry1;
		$rst1 = $dbConn->query($qry1);
		$page_total = mysqli_num_rows($rst1);
		$page_last = ceil($page_total / $scale);


//echo $qry1;
		
		$qry1 = "SELECT * FROM dong_mlist2 AS a WHERE 1=1 $name_qry $name_qry1 order by wdate desc LIMIT $start, $scale";
...
//echo $qry1;
		...
		while ($row1 = $rst1->fetch_assoc()) {
			...
			echo "<tr>
				...
				<td>".$row1[tel_num]."</td>
				<td>".$row1[area]."</td>
				<td align='center'><input type=checkbox name=chksub[]  value='".$row1[chk_sub]."' class='chk' onclick='GoCheck($row1[seq_no]);' $checkyn></td>
	...
	function board_pageNavigation(){
      ...
      if($page_total>(($page+1)*$scale*$page_scale))
              ...
              echo "&nbsp;<a href='$PHP_SELF?start=$n_start&$Parameter_value'>></a></a>&nbsp;";
              echo "<a href='$PHP_SELF?start=$last_start&$Parameter_value'>>></a>";
              }
      }
   }// pageNavigation function end
	$qry2 = "select count(*) as cnt from dong_mlist2 where chk_sub='0' && chk_send = 1";
	...
	<div id="contentwrapper" class="reservationDetailForm">
		<div class="main_content">
			...
			<form action="<?= $PHP_SELF ?>" id="form_mail" name="form_mail" method="post" Enctype="multipart/form-data">
			    ...
			    <input type="hidden" name="chkvalue1" id="chkvalue1" >		
				<div class="row">
					...
					<table id="level4" class="txt_12" width="98%" align="center" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td bgcolor="#FFFFFF" height="30"><? if ($row2[cnt] !="0") {?> | <font color=red>보낸이메일갯수 <?=$row2[cnt]?></font><? } ?></td>
						</tr>
				  ...
				  <table class="table table-bordered table-condensed">
				   ...
				   <tr>
					...
					<?php if ($user_dbinfo[userid] == "wincom00") { ?>
					<tr>
