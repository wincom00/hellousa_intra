									...
									<div class='col-sm-3'>
										<div class='input-group input-group-sm'>       
										   ...
										   if ($gscode == $row1[sub_eCode]) {
										         echo "<input type='radio' name='roomnumber' value='$s' checked onClick='openwin(\"$row1[grand_eCode]\",\"$row1[sub_eCode]\",\"$pcode\",\"$st\")'>방갯수 : $rcnt[rcnt1] 개";
										   } else {
											   echo "<input type='radio' name='roomnumber' value='$s'  onClick='openwin(\"$row1[grand_eCode]\",\"$row1[sub_eCode]\",\"$pcode\",\"$st\")'>방갯수 : $rcnt[rcnt1] 개";
...
<body>
    ...
    <script>
        ...
        function openwin(g_code,s_code,p_code,st) { 
		   ...
		   window.open("guide_assign_customer1.php?g_code="+g_code+"&s_code="+s_code+"&p_code="+p_code+"&st="+st,"customer","width=1050,height=700,scrollbars=1");
	    }
