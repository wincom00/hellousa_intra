...
pt = Object.assign({}, pt || {}, {
	...
	initProductDetailForm: function initProductDetailForm() {
		...
		for (var i = 0; i < scope.length; i++) {
			...
			var addOptBtns = self.find('.js-addOpt')
			var removeOptBtns = self.find('.js-removeopt')
			...
			addOptBtns.off('click.addOpt').on('click.addOpt', this.addOpt)
            removeOptBtns.off('click.removeOpt').on('click.removeOpt', this.removeOpt)
	...
	addOpt: function addOpt(e) {
		var btn = $(this)
		var set = $(btn.closest('tr').find('.js-optSet')[0])
		var parent = set.closest('td')
		set.clone(true)
		.find('.js-removeopt').removeClass('hidden')
		.end().find(':input').val('')
		.end().appendTo(parent)
	},
    removeOpt: function removeOpt(e) {
		var btn = $(this)
		var actionActive = !btn.hasClass('hidden')
