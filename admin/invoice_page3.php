...
<br />
<form name=print id=print action='invoice_page3.php?division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&r_code=<?=$r_code?>' method=post enctype="multipart/form-data">
  <input type=hidden name=r_code id='r_code' value="<?= $r_code ?>">
 	
		<!-- invoice page -->
	<div id="invoice">
		
		<div class="invoice1 overflow-auto"  style="page-break-before: always;">
				...
				<main>
				   <div class="row contacts">
					...
					<?php
						 ...
						 $cc_pmt_fee_qry = "select sum(payment * 0.04) as cc_proc_fee from payment_history where reserveCode = '$r_code' && payment_status = 'DONE' && pay_method = 'bcreditcard'";
						 ...
						 $cc_pmt_fee_qry = "select sum(payment * 0.04) as cc_proc_fee from payment_history where reserveCode = '$r_code' && payment_status = 'DONE' && pay_method = 'creditcard'";
						 $cc_pmt_fee_rst = $dbConn->query($cc_pmt_fee_qry);
						 $row2 = $cc_pmt_fee_rst->fetch_assoc();
...
<script>
		...
		{ 
		   ...
		   $("#print").attr("action","invoice_p3.php?division=3&pdx=2&sub=15&r_code="+r_code+"");
		   $("#print").submit();
		} 
