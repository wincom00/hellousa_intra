			...
			<form action="<?= $PHP_SELF ?>?estimateCode=<?=$estimateCode?>&division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&st=<?=$st?>&ty=<?=$ty?>&pcode=<?=$pcode?>&pricet=<?=$pricet?>" name="frmreserve" id="frmreserve" method="post" Enctype="multipart/form-data" autocomplete="false" >
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						...
						<?php
									$qryr = "select a.* from reserve_info a  where  a.reserveCode = '$estimateCode'&& a.parent='SUB' && dis_code not in ('pick','send' ) order by a.stDate,pos asc";
									//echo $qryr;
						...
						?>
						<?php
									$qry1 = "select * from product_details_local where p_code = '$prodInfo[p_code]' && dis_code not in ('pick','send' ) 
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											...
											<option value="PICKUP" <?php if ($reserve_pick[p_code]== 'PICKUP') { ?> selected <?php } ?> >JFK 픽업</option>
											<option value="EWRPICKUP" <?php if ($reserve_pick[p_code]== 'EWRPICKUP') { ?> selected <?php } ?> >EWR 공항픽업</option>
											<option value="LAPICKUP" <?php if ($reserve_pick[p_code]== 'LAPICKUP') { ?> selected <?php } ?>>LA 공항픽업</option>
											<option value="LVPICKUP" <?php if ($reserve_pick[p_code]== 'LVPICKUP') { ?> selected <?php } ?>>LAS VEGAS 공항픽업</option>
											<option value="SFOPICKUP" <?php if ($reserve_pick[p_code]== 'SFOPICKUP') { ?> selected <?php } ?>>SFO 공항픽업</option>
											<option value="SLCPICKUP" <?php if ($reserve_pick[p_code]== 'SLCPICKUP') { ?> selected <?php } ?>>SLC 공항픽업</option>
											<option value="ANCPICKUP" <?php if ($reserve_pick[p_code]== 'ANCPICKUP') { ?> selected <?php } ?>>ANC 공항픽업</option>
											<option value="ATLPICKUP" <?php if ($reserve_pick[p_code]== 'ATLPICKUP') { ?> selected <?php } ?>>ATL 공항픽업</option>
										</select>
										</div>
									</div>
								</div>
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											...
											<option value="SENDING" <?php if ($reserve_send[p_code]== 'SENDING') { ?> selected <?php } ?>>JFK 샌딩</option>
											<option value="EWRSENDING" <?php if ($reserve_send[p_code]== 'EWRSENDING') { ?> selected <?php } ?>>EWR 공항샌딩</option>
											<option value="LASENDING" <?php if ($reserve_send[p_code]== 'LASENDING') { ?> selected <?php } ?>>LA 공항샌딩</option>
											<option value="LVSENDING" <?php if ($reserve_send[p_code]== 'LVSENDING') { ?> selected <?php } ?>>LAS VEGAS 공항샌딩</option>
											<option value="SFOSENDING" <?php if ($reserve_send[p_code]== 'SFOSENDING') { ?> selected <?php } ?>>SFO 공항샌딩</option>
											<option value="SLCSENDING" <?php if ($reserve_send[p_code]== 'SLCSENDING') { ?> selected <?php } ?>>SLC 공항샌딩</option>
											<option value="ANCSENDING" <?php if ($reserve_send[p_code]== 'ANCSENDING') { ?> selected <?php } ?>>ANC 공항샌딩</option>
											<option value="ATLSENDING" <?php if ($reserve_send[p_code]== 'ATLSENDING') { ?> selected <?php } ?>>ATL 공항샌딩</option>
										</select>
										</div>
									</div>
								</div>
						...
						<tr>
							...
							<td colspan="3" class="no-left-border">
								<div class="row">
									...
									<div class="col-sm-6">
										<button type="button" class="btn btn-xs btn-block btn-default js-calculateFinalAmount" <?php if (!$estimateCode) {?>disabled  <?php } ?> onClick="calc(1)">최종계산</button>
									</div>
								</div>
							</td>
