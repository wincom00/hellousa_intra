<?php
    ...
    if ($mode =="save") {
        for($i=0; $i<count($seqNo); $i++) {
            ...
            if ($gcode[$s] == "") {
                ...
                $total_eventCode = "HSP".date("His", $sec) . $hs.$total_eventNum;
                ...
                $qry2 = "insert into tour_master (grand_eNum, grand_eCode, p_code, p_name, tour_pcnt, stDate, edDate, r_status, ev_status, s_pcode, etc_memo, ev_memo, chk_ass, userid, wdate)
                         values ('$total_eventNum', '$total_eventCode', '$pcode[$s]', '$pname[$s]', '$acnt[$s]', '$sdate[$s]', '', '', '', '', '', '', '', '$user_dbinfo[userid]', now())";
                 $rst2=$dbConn->query($qry2);
            } else {
                 $qry2 = " update tour_master set s_pcode = '$productSub' where grand_eCode = '$gcode[$s]' ";
                 $rst2=$dbConn->query($qry2);
            }
        }
        Misc::jvAlert("저장 완료!!!");
        echo "<meta http-equiv='refresh' content='0; url=./eventbs_list.php?division=5&pdx=$pdx&sub=$sub'>";
        exit;
    }
    ...
    /* =========================================================================
       [핵심 수정] 출력 함수 printSingle
       - 'all' 모드 시: 기간 내 날짜를 루프 돌며 product_master의 유효기간(p_vstart~p_vend) 체크
       ========================================================================= */
    function printSingle(){
            ...
            $view_mode = isset($_POST['view_mode']) ? $_POST['view_mode'] : 'reserved';
            ...
            if ($view_mode == 'all') {
                // =============================================================
                // [MODE: 전체 스케줄 보기]
                // 전략: PHP에서 시작일~종료일 루프를 돌면서, 해당 날짜에 유효한(p_vstart~p_vend) 상품을 조회
                // =============================================================
                ...
                for ($i = $startTS; $i <= $endTS; $i += 86400) {
                    ...
                    // 쿼리 설명:
                    // 1. product_master(b) 기준
                    // 2. 날짜 범위 체크: $currDate가 p_vstart와 p_vend 사이에 있어야 함
                    // 3. 요일 체크: p_week 컬럼에 현재 요일($currWeek)이 포함되어 있어야 함
                    // 4. tour_master(c), reserve_info(a)는 LEFT JOIN으로 정보만 가져옴
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <label class="radio-inline" style="margin-right:20px;">
                                    ...
                                    <?php if(empty($_POST['view_mode']) || $_POST['view_mode']=='reserved') echo 'checked'; ?>> 
                                    <strong>예약된 상품만 보기</strong> (기본)
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="view_mode" value="all" 
                                    <?php if($_POST['view_mode']=='all') echo 'checked'; ?>> 
