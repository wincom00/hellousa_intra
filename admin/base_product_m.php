				...
				<table class="table table-bordered table-condensed ptTable formDetail">
					<tbody>
						...
						<tr>
							...
							<td colspan="10">
								<input type="text" class="form-control" name="prod_cat_name" placeholder="상품분류명칭" value='<?=$prodInfo['p_cat_name']?>'>
							</td>
						</tr>
						<?php } ?>
						<?php if ($ty == 1) { ?>
						<tr>
							...
							<td colspan="10" align="left" bgcolor="#FFFFFF">
								&nbsp;<label class="checkbox-inline">
									<input type="checkbox" name="m_guidechk" value="V" <?php if($prodInfo['m_guidechk'] == "V") echo "checked"; ?>> 메인가이드상품
								</label>
								<span class="text-muted small">로컬상품만 사용</span>
							</td>
						</tr>
						<?php } ?>
						<!--<tr>
						...
						<tr>
							...
							<td colspan="10" class="form-inline">
								<select class="form-control fst1" name="area1" id="area1">
									<option value="">지역선택
									<?=printBaseCode_first('P02',$prodInfo['c_code1'])?>
								</select>
								
								<select class="form-control fst2" name="area2" id="area2">
									<option value="">지역선택2</option>
									<?=printBaseCode_hsecond('P02',$lvcode2,$prodInfo['c_code2'])?>
								</select>
						...
						<tr>
								...
								<td colspan="10">
									<table border=0>
										<tr>
										<?php
											$cr_qry1 = "select * from code_base where lvcode1 = 'P02' && lvcode2 <> '00' && lvcode3 <> '00' && lvcode4 <> '00' order by lvcode2 asc";
											$cr_rst1 = $dbConn->query($cr_qry1);
						...
						<tr>
							...
							<td colspan="4">
								<select class="randcomp" name="p_own" id="p_own">
									...
									<option value="hello" selected>Tour Hello USA</option>
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">옵션설정 &nbsp;<button type="button" class="btn btn-default btn-xs js-addOpt"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
							<td colspan="10">
							   <?php
									$qry1 = "select * from product_opt where p_code = '$prodInfo[p_code]'";
									
									$rst1 = $dbConn->query($qry1);
									$cnt = $rst1->num_rows;
									while($opt_row = $rst1->fetch_assoc()):
						
							   ?>
									<div class="form-inline js-optSet">
										<select class="form-control" name="optLoc[]">
											<option value="">옵션선택</option>
											<?=optBaseCode($opt_row['opt_code'])?>
										</select>
										
										<button type="button" class="btn btn-default btn-xs js-removeopt"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button>
									</div>
							   <?php
