<?php
	...
	if ($mode == "send_email") {
			$sbj = "[동부투어] 정상적으로 예약번호 ".$r_code." 으로 예약이 접수되었습니다.";

			//$content= file_get_contents( "https://wwww.dongbutour.online/invoice_m.php?division=3&pdx=2&sub=15&r_code=".$r_code."");
			$content= file_get_contents("http://dongbutour.online/invoice_m.php?division=3&pdx=2&sub=15&r_code=".$r_code."");
			...
			$ret= mailsend_h('tourhellousa@gmail.com',$sbj,$msg,$attachment1,$attachment2,$attachment3,$attachment4);
			} else {
			$ret= mailsend_h('tourhellousa@gmail.com',$sbj,$msg,$attachment1,$attachment2,$attachment3,$attachment4);
			}
				...
				<table style='width: 100%;line-height: 18px;text-align: left;border: 1px solid #aaa;font-size: 13px;'>
					<tbody>
						...
						<tr style="background: #eee;font-weight: bold;text-align: center;padding: 10px;border: 1px solid #aaa;">
							...
							<td colspan="14" style="background: #fff;padding: 5px;text-align: left;"><?=$revInfo[base_rate]?> <?php echo number_format($revInfo[last_total]);?> </td>
						</tr>
						<tr style="background: #eee;font-weight: bold;text-align: center;padding: 10px;border: 1px solid #aaa;">
