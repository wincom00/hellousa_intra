...
$incPath = dirname(__FILE__) . "/admin/include/inc_base.php";
if (file_exists($incPath)) {
    include_once $incPath;
} else {
    $incPath2 = $_SERVER['DOCUMENT_ROOT'] . "/admin/include/inc_base.php";
...
if (!isset($dbConn) || !($dbConn instanceof mysqli)) {
    echo "<div style='padding:20px;font-family:Arial;color:#b00'>mysqli 연결(\$dbConn)이 없습니다. inc_base.php에서 mysqli로 연결되었는지 확인하세요.</div>";
    exit;
}
...
if (!function_exists('esc')) {
    function esc($s) {
        ...
        $s = (string)$s;
        return $dbConn->real_escape_string($s);
    }
}
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    if ($kind === 'product_reservations') {
        ...
        $cntRow = db_one("
            ...
            FROM reserve_info
            WHERE parent='MAIN'
              AND {$DATE_COL} BETWEEN '{$sDate}' AND '{$eDate}'
              AND rev_status IN ('READY','DONE','CANCEL')
              AND p_code='{$p_code}'
        ");
...
<style>
  ...
  .topArea { flex: 1 1 auto; min-height: 0; overflow: hidden; display: flex; flex-direction: column; }
