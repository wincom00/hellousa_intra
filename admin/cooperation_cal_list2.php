<?php
	...
	if ($mode1 == "save") {
		...
		for($i=0; $i<count($seqNo); $i++)
		{
			...
			//$calMemo = $calMemo."<br/>".$pmemo;
			$qry1= "insert into rand_pay 
											( 
											rand_id, 
											reserveCode,
											rand_date,
	...
	function printsettle(){
		    ...
		    $qry1 = "select a.seq_no as seqr,a.*,b.*
				...
				&& a.reserveCode is not null && b.rev_status!='CANCEL'  $flag_qry  order by b.stDate,a.wdate desc";
			...
			//echo $qry1;
				...
				echo "<tr>
							<td align='center'><input type='checkbox' name=seqNo[] id='seq_rno' value='$k' class='form-control' onclick='GoCheck($k);'><input type=hidden name=amta[] value='$row1[amt]'><input type=hidden name=balamt[] value='$row1[cur_amt]'><input type=hidden name='seq[]' id='seqr' value='$row1[seqr]'>
							<input type=hidden name='rdate[]' id='rdate' value='$rdate'>
							<input type=hidden name='sdate[]' id='sdate' value='$sdate'></td>
							<td align='center'>$rdate<br/>$row1[reserveCode]<input type=hidden name='sreserveCode[]' id='sreserveCode' value='$row1[reserveCode]'><input type=hidden name='sdivi[]' id='sdivi' value='$row1[money_type]'><input type=hidden name='randid[]' id='randid' value='$row1[part_id]'></td>
							<td align='center'>$row1[p_cnt]</td>
							<td><a href=javascript:openwin1('$row1[reserveCode]','$row1[pricet]')>$row1[p_name]<br/>$row1[stDate]~$row1[edDate]</a></td>
							<td align='center'><a href=javascript:openwin1('$row1[reserveCode]','$row1[pricet]')> $recus[traveler_nm]</a></td>
							
							<td align='center'><a href=javascript:openwin1('$row1[reserveCode]','$row1[pricet]')> $row1[stDate]</a></td>
							...
							<td><input type=hidden name='pmemo[]' id='pmemo' value='$row1[p_memo]'>$row1[p_memo]<br>$paym</td>
							<td align='center'><button type=button name=in[] value='$row1[part_id]/$row1[reserveCode]' class='btn btn-xs btn-default js-pay' onClick=javascript:openwin('$row1[part_id]','$row1[reserveCode]','$row1[seqr]','$rdate','$sdate')>PAY</button>
							<button type=button name=in2[] value='$row1[part_id]|$row1[reserveCode]|$row1[seqr]' class='btn btn-xs btn-default js-rpay' >RESET</button></td>
						</tr>";
			...
			<div class="row">
				<div class="col-sm-12 col-md-12">
					...
					<input type="hidden" name="mode" id="mode" value="save">
						<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
                            <tbody>
								...
								<tr>
									<td colspan="10">  
                                    <td  class="active text-center formHeader">수신이메일입력<br/><font color=red>"이메일을 입력하지 않으면 협력사 기본이메일로 발송됩니다."</font></td>
                                    <td colspan="2">   
                                        <input type="text" name="iemail" id="iemail" data-date-format='yyyy-mm-dd' class="form-control" aria-label="이메일" placeholder="이메일" autocomplete="off" value="">
                                    </td>
								</tr>
                            </tbody>
                        </table>
					...
					<input type="hidden" name="mode1" id="mode1" value="save">
						<div class="row">
							<div class="col-sm-12">
								<table id="js-settleTable" class="table table-striped table-bordered table-hover table-condensed js-settleTable">
									<thead>
										<tr>
										...
										<tr>                    			
											...
											<td colspan="6">
												...
												<select class="form-control" name="paymentMethod" id="paymentMethod">
													...
													<option value="wireab">해외송금</option>
												</select>
											</td>
											...
											<td colspan="6">
												<select class="form-control" name="bankSelect" id="bankSelect">
													...
													<option value="boa">BOA Bank</option>
													<option value="pnc">PNC Bank</option>
												    <option value="han1">한국 계좌</option>
										            <option value="han2">한국계좌(지인)</option>
    ...
    <script>
		$(document).ready(function () {
			...
			$('body').on('click', '.js-in', function () {
				...
				{
					f.target = "invoice";
					f.action = "in.php?rand_id=" + rand_id + "&s_date1=" + s_date1 + "&s_date2=" + s_date2+"&iemail="+iemail;
