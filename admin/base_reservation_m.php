  ...
  // include "include/inc_base.php";
	...
	} else if(($estimateCode) && ($cp==''))
	{
			 ...
			 if ($prodInfo[p_type] == 5) {
				 $ccomp_info=getCTRandInfo($estimateCode);
				 $dcomp_info=getDTRandInfo($estimateCode);
				 $cm_info =  getCMRandInfo($estimateCode);
			 } else {
				 $ccomp_info=getCRandInfo($estimateCode);
				 $dcomp_info=getDRandInfo($estimateCode);
				 $cm_info =  getCRMandInfo($estimateCode);
			...
			//exit;
			 ...
			 $qry1 = "select * from reserve_opt where reserveCode = '$reserve_info[reserveCode]'order by wdate asc";
	
			 $rst1 = $dbConn->query($qry1);
			 $totoptamt = 0;
			 while($p_row = mysqli_fetch_assoc($rst1)){

				$totoptamt = $totoptamt + $p_row[opt_amt];
...
?>
	...
	<div id="contentwrapper" class="reservationDetailForm">
		<div class="main_content">
			...
			<form action="<?= $PHP_SELF ?>?estimateCode=<?=$estimateCode?>&division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&st=<?=$st?>&ty=<?=$ty?>&pcode=<?=$pcode?>&pricet=<?=$pricet?>" name="frmreserve" id="frmreserve" method="post" Enctype="multipart/form-data" autocomplete="false" >
				...
				<input type="hidden" name="settlrpt" id="settlrpt" value="<?=$reserve_info[settle_report]?>">
				
				<div class="row no-nav">
					<div class="col-sm-6">
						<ul class="pagination non-nav">
						
							<li <?php if (($reserve_info[rev_status]=='READY') && ($revst != 'WAIT')) { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>예약접수</span></li>
							
							<li <?php if (($reserve_info[rev_status]=='DONE')) { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>예약확정</span></li>
							
							<li <?php if ($reserve_info[rev_status]=='CANCEL')  { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>예약취소</span></li>
				...
				<div class="row no-nav">
					<div class="col-sm-6">
						<ul class="pagination non-nav">
							<li <?php if ($reserve_info[payment_st]=='READY') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>미납</span></li>
							<li <?php if ($reserve_info[payment_st]=='PPAY') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>부분완납</span></li>
							<li <?php if ($reserve_info[payment_st]=='DONE') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>class="disabled"><span>완납</span></li>
							<li <?php if ($reserve_info[payment_st]=='OPAY') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>class="disabled"><span>환불</span></li>
						</ul>
					</div>
					<div class="col-sm-6 text-right">
					  <?php if (($estimateCode)  && ($cp =='') && ($reserve_info[settle_report] == '0')) { ?>
							<button type="button" class="btn btn-xs btn-default js-rr" onClick="go_submit()">예약저장</button>
							
							<button type="button" <?php if (($reserve_info[rev_status]=='DONE') || ($reserve_info[rev_status]=='CANCEL') && ($reserve_info[rev_status]!=''))  { ?> class="btn btn-xs btn-default js-done disabled" disabled <? } else { ?> class="btn btn-xs btn-default js-ccr" <?php } ?> onClick="go_corder()">예약확정</button>
							...
							<?php if ($prodInfo[p_type] == 5) { ?>
							   ...
							   <button type="button" class="btn btn-xs btn-default js-prn2" onClick="javascript:openwin2('<?=$estimateCode?>')">확정서출력(로컬)</button>
							   <button type="button" class="btn btn-xs btn-default js-ttrpt" onClick="javascript:openwin3('<?=$estimateCode?>')">투어리포트</button>
							   <button type="button" class="btn btn-xs btn-default js-settle" onClick="go_settle()">정산보고</button>
							<?php } ?>
					  <?php } else if (($reserve_info[settle_report] == '1') || ($reserve_info[settle_report] == '2')) { ?>
								<button type="button" class="btn btn-xs btn-default js-rr"" disabled>예약접수</button>
								...
								<?php if ($prodInfo[p_type] == 5) { ?>
								   ...
								   <button type="button" class="btn btn-xs btn-default js-prn2" onClick="javascript:openwin2('<?=$estimateCode?>')">확정서출력(로컬)</button>
								   <button type="button" class="btn btn-xs btn-default js-ttrpt" onClick="javascript:openwin3('<?=$estimateCode?>')">투어리포트</button>
								   <button type="button" class="btn btn-xs btn-default js-settle" disabled>정산보고</button>
								<?php } ?>
					   ...
					   <?php } else { ?>
								...
								<button type="button" class="btn btn-xs btn-default js-prn2" disabled>확정서출력(로컬)</button>
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail js-base">
					<tbody>
						...
						<tr>
							...
							<td colspan="2" class="active text-center formHeader">방갯수&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-sm js-addroom1" ><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button> &nbsp;<button type="button" class="btn btn-default btn-sm js-rRoom1" ><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button></td>
							<td colspan="2"><div class="row">
									<div class="col-sm-12">
									<?php if ( ($cp =='')) { ?>
									    <div class="input-group input-group-sm">
											<input type="number" step='0.5' min=0 name='rcnt1' id='rcnt1' class="form-control text-right rcnt1" aria-label="개" value="<?php if ($reserve_info[room_cnt]) {  echo $reserve_info[room_cnt]; } else if ($consultCode){ echo $consult_info[room_cnt]; } else { ?>0<?php } ?>"
											/>
											<span class="input-group-addon">개</span>
										</div>
									<?php } else { ?>
										 <div class="input-group input-group-sm">
											<input type="number"  step='0.5' min=0 name='rcnt1' id='rcnt1' class="form-control text-right rcnt1" aria-label="개" value="<?php if ($consultCode){ echo $consult_info[room_cnt]; } else { ?>0<?}?>"
											/>
											<span class="input-group-addon">개</span>
										</div>
									 <?php } ?>
							...
							<td colspan="2" class="active text-center formHeader">총모객인원</td>
						...
						<tr>
							...
							<td colspan="2" class="active text-center formHeader">여행인원&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-sm js-addTraveler1" ><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button> &nbsp;<button type="button" class="btn btn-default btn-sm js-rTraveler1" ><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button></td>
							<td colspan="2">
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
							...
							<td colspan="2" class="active text-center formHeader">담당자</td>
							<td colspan="2"><div class="text-center"><?=$reserve_info[userid]?></div></td>
						</tr>
					</tbody>
				</table>
				<table id="example" class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					...
					?>
						<tr>
							<td colspan="5" class="active text-center formHeader fullWidth">예약상세정보 &nbsp;<button type="button" class="btn btn-default btn-sm js-copy" >복사</button></td>
						</tr>
								...
								<div class="row">
									<div class="col-sm-10">
										<div class="input-group input-group-md">
											...
											<select class="rand" name="rand" id="rand" autocomplete='off'>
					...
					<?php } else { ?>
						<tr>
							<td colspan="4" class="active text-center formHeader fullWidth">예약상세정보 &nbsp;<button type="button" class="btn btn-default btn-sm js-copy" >복사</button></td>
						</tr>
						<tr class="innerTable1">
							<td  class="active text-center formHeader">여행자정보</td>
							<td width='20%'>
								<!-- <input type="text" class="form-control" placeholder="이름"> -->
								<div class="row">
									<div class="col-sm-9">
				...
				<table id="example1" class="table table-bordered table-condensed gridSixteen reserveTable formDetail innerTable">
						...
						<?php
							...
							$rstt = $dbConn->query($qryt);
							$cnt = $rstt->num_rows;
							if ($cnt > 0) {
								$num = 0;
							    while($rowt = $rstt->fetch_assoc()):
								  if (($num == 0) && ($cp =='')) {
											  ...
											  <tbody id="travelerTableBody">
												<tr class='innertr'>
														...
														<td >
															...
															<input type="text" class="form-control" aria-label="영문이름" name="t_ename[]" id="t_ename" autocomplete=off placeholder="영문이름" value="<?=$rowt[traveler_enm]?>"/>
														...
														<td >
															...
															<input type="text" class="form-control" aria-label="여권번호" name="t_passnum[]" id="t_passnum" autocomplete=off placeholder="여권번호" value="<?=$rowt[pass_num]?>"/>		
														</td>
														<td >
									...
									<tbody id="travelerTableBody">
												<tr class='innertr'>
														...
														<td >
															...
															<input type="text" class="form-control" aria-label="영문이름" name="t_ename[]" id="t_ename" autocomplete=off placeholder="영문이름" value=""/>
														...
														<td >
															...
															<input type="text" class="form-control" aria-label="전화번호" name="t_phone[]" id="t_phone" autocomplete="off"
															value=""/>
															<input type="text" class="form-control" aria-label="여권번호" name="t_passnum[]" id="t_passnum" autocomplete=off placeholder="여권번호" value=""/>		
														</td>
														<td >
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable1 formDetail">
				  <tbody >
				  	    <tr>
							<td  class="active text-center formHeader">옵션설정 &nbsp;기준</td>
							<td class="formHeader">
                                    <div class="form-inline js-optSet1" >
										<select class="form-control selectpicker js-optLoctemp"  style="width:300px;" name="optLoctemp"  ID="optLoctemp" multiple="multiple" data-live-search="true">
										
											<?=optBaseCode4('',$pcode,'')?>
										</select>
										<input type="text"  name='optamttemp' id='optamttemp' class="form-control text-right js-opttemp" value="0" style="width:200px;"/>
										
										<button type="button" class="btn btn-default btn-xs js-batch">일괄옵션복사</button>
									</div>
				        ...
				        <tr>
							<td  class="active text-center formHeader">옵션설정 &nbsp;<button type="button" class="btn btn-default btn-xs js-addOpt"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
							<td class='optsec'>
							<?php
									$qry1 = "select p_code,opt_code,sum(opt_price) as oprice,tnm from reserve_opt where reserveCode='$reserve_info[reserveCode]' group by tnm";
								   // echo $qry1;2024-10-22
									//exit;
									$rst1 = mysqli_query($dbConn,$qry1);
									$cnt11 = mysqli_num_rows($rst1);
									if ($cnt11==0) {
										$cnt12 = 1;
									} else  {
										$cnt12 = $cnt11;
									...
									$k =0;
									while($opt_row = mysqli_fetch_assoc($rst1)):
									//echo $opt_row['opt_code'];
						            
						          ?>
									<div class="form-inline js-optSet" >

										<select class="form-control  js-optLoc" style="width:200px ;" name="optLoc<?=$k?>[]"  ID="optLoc<?=$k?>" multiple="multiple" data-live-search="true">
								
											<?=optBaseCode4($reserve_info[reserveCode],$opt_row[p_code],$opt_row[tnm])?>
										</select>
										<input type="text"  name='optamt[]' id='optamt' class="form-control text-right js-opt" value="<?php if (($opt_row[oprice])) {  echo $opt_row[oprice]; }  ?>"	/>
										<select class="form-control js-optamt" name="optnm[]" id="optnm">
										   <optio0n value="">선택이름</option>
										  <?php echo getReserveTrIn2($reserve_info[reserveCode],$opt_row['tnm']); ?>
										</select>
										<button type="button" id='btnrev' class="btn btn-default btn-xs js-removeopt"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button><b><hr style='border-style: solid; border-color: red;'/></b>
						       ...
						       ?>
									<div class="form-inline js-optSet" >
										<select class="form-control selectpicker js-optLoc"  style="width:200px;" name="optLoc<?=$k?>[]"  ID="optLoc<?=$k?>" multiple="multiple" data-live-search="true">
										
											<?=optBaseCode4('',$pcode,'')?>
										</select>
										<input type="text"  name='optamt[]' id='optamt' class="form-control text-right js-opt" value="0"/>
										<select class="form-control js-optamt" name="optnm[]" id="optnm" >
										   <option value="">선택이름</option>
										  <?php echo getReserveTrIn($reserve_info[reserveCode]); ?>
										</select>
										<button type="button" class="btn btn-default btn-xs js-removeopt hidden"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button>
									</div>
							   <?php
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable1 formDetail">
				  <tbody>
						...
						<tr>
							...
							<td colspan="4">
								<div class="row">
									...
									<div class="col-sm-12">
										<select class="form-control comp" name="tourcomp" id="tourcomp" autocomplete="false" >
											<option value="">- 업체를 선택하세요 -
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">커미션-업체</td>
							<td colspan="4">
								<div class="row">
									<!--<div class="col-sm-6">
										<select class="form-control areaf" name="tourRegion">
									...
									<div class="col-sm-12">
										<select class="form-control comp" name="tourcm" id="tourcm" autocomplete="false" >
											<option value="">- 업체를 선택하세요 -
											<?=printCompanySelect1($cm_info[part_id])?>

										</select>
									</div>
								</div>
							</td>
							<td colspan="2" class="active text-center formHeader">커미션금액</td>
							<td colspan="8">
								<div class="row">
									<div class="col-sm-3">
										<!-- <input type="text" id="" name="startDate" class="inpubase" placeholder="금액" value=""/> -->
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액</span>
											<input type="text" name="cramt" id="cramt" class="form-control text-right" aria-label="금액" value="<?=$cm_info[amt]?>"/>
										</div>
									</div>
									<div class="col-sm-5">
										<!-- <input type="text" id="" name="endDate" class="inpubase" placeholder="금액내용" value=""/> -->
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액내용</span>
											<input type="text" name="cramtmemo" id="ramtmemo" class="form-control" aria-label="금액내용" value="<?=$ccomp_info[p_memo]?>"/>
										</div>
									</div>
									<div class="col-sm-4">
									<button type="button" class="btn btn-primary  js-removecomp2" >DELETE</button>
									</div>
								</div>
							</td>
						</tr>
						...
						<!--<tr>
							<td colspan="2" class="active text-center formHeader">지급-업체 &nbsp;<button type="button" class="btn btn-default btn-xs js-addRand"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
							<td colspan="10">
									...
									<div class="form-inline js-pickupSet">
									  <table class="table table-bordered table-condensed gridSixteen reserveTable1 formDetail">
                                        <tr>
											...
											<td class="active text-center formHeader">지급할 NET 금액</td>
											<td>
												<div class="input-group input-group-sm">
													<span class="input-group-addon">금액</span>
				...
				<table id="addRandtab" class="table table-bordered table-condensed gridSixteen reserveTable1 formDetail">
				  <tbody id="Rbody">
				  <?php 
				        if ($prodInfo[p_type] == 5) {
                             $tab = "rand_company_tmp";

						} else {
							
							 $tab = "rand_company";
						}
						$cr_qry1 = "SELECT * FROM  $tab WHERE reserveCode='$estimateCode' && money_type='debit' && p_memo !='발권'";
						$cr_rst1 = $dbConn->query($cr_qry1);

						$cr_num1 = 1;

						$cntr= $cr_rst1->num_rows;
						if ($cntr > 0) {
						...
						<tr class="rand1">
							...
							<td colspan="4">
								<div class="row">
									...
									<div class="col-sm-12">
										<select class="form-control comp2" name="tourcomp1[]" id="tourcomp1" autocomplete="false">
											<option value="">- 업체를 선택하세요 -
										<?=printCompanySelect($cr_row1[part_id])?>
										</select>
									</div>
								</div>
							</td>
							...
							<td colspan="8">
								<div class="row">
									<div class="col-sm-3">
										...
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액</span>
											<input type="text" name="pamt[]" class="form-control text-right" aria-label="금액" value="<?=$cr_row1[amt]?>"/>
										</div>
									</div>
									<div class="col-sm-5">
										...
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액내용</span>
											<input type="text" name="pamtmemo[]" class="form-control" aria-label="금액내용" value="<?=$cr_row1[p_memo]?>"/>
										</div>
									</div>
									<div class="col-sm-4">
							...
							<td colspan="8">
								<div class="row">
									<div class="col-sm-3">
										...
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액</span>
											<input type="text" name="pamt[]" class="form-control text-right" aria-label="금액" value=""/>
										</div>
									</div>
									<div class="col-sm-5">
										...
										<div class="input-group input-group-sm">
											<span class="input-group-addon">금액내용</span>
											<input type="text" name="pamtmemo[]" class="form-control" aria-label="금액내용" value=""/>
										</div>
									</div>
									<div class="col-sm-4">
										<button type="button" class="btn btn-primary  js-removecomp1" >DELETE</button>
				...
				<table  class="table table-bordered table-condensed gridSixteen reserveTable1 formDetail">
				  <tbody>
				        <tr>
							<td colspan="2" class="active text-center formHeader">아웃바운드 첨부파일</td>
							<td >
							   
								&nbsp;<? if($reserve_info['userfile1']): ?><a href='download.php?filename=<?= $reserve_info['userfile1'] ?>'><?= $reserve_info['userfile1'] ?></a><? endif; ?><input type="file" name="userfile1"  class='form-control'><input type="checkbox" id="photo_del1" name="photo_del1" value="1">삭제 
							</td>
							<td >
							   
								&nbsp;<? if($reserve_info['userfile2']): ?><a href='download.php?filename=<?= $reserve_info['userfile2'] ?>'><?= $reserve_info['userfile2'] ?></a><? endif; ?><input type=file name=userfile2  class='form-control' value="<?= $reserve_info['userfile2'] ?>"><input type="checkbox" id="photo_del2" name="photo_del2" value="1">삭제 
							</td>
							<td >
							   
								&nbsp;<? if($reserve_info['userfile3']): ?><a href='download.php?filename=<?= $reserve_info['userfile3'] ?>'><?= $reserve_info['userfile3'] ?></a><? endif; ?><input type=file name=userfile3  class='form-control' value="<?= $reserve_info['userfile3'] ?>"><input type="checkbox" id="photo_del3" name="photo_del3" value="1">삭제 
							</td>
							<td colspan="4">
							   
								&nbsp;<? if($reserve_info['userfile4']): ?><a href='download.php?filename=<?= $reserve_info['userfile4'] ?>'><?= $reserve_info['userfile4'] ?></a><? endif; ?><input type=file name=userfile4  class='form-control' value="<?= $reserve_info['userfile4'] ?>"><input type="checkbox" id="photo_del4" name="photo_del4" value="1">삭제 
							</td>
						</tr>
						<tr>
							<td colspan="2" class="active text-center formHeader">진행사항</td>
							<td colspan="14">
								<textarea class="form-control pmemo" rows="15" name="pmemo" ><?=$reserve_info[progress]?></textarea>
							</td>
						</tr>
						<tr>
				...
				<table id="air" class="table table-bordered table-condensed gridSixteen">
					<tbody id="airtbody">
						<tr>
							<td colspan="16" class="active text-center formHeader fullWidth"><input name='reissue' type='checkbox' value='Y' <?php if ($reserve_info[re_issue]=='Y') {  ?> checked <?php } ?>>reissue&nbsp;&nbsp;&nbsp;&nbsp;항공예약 &nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-xs js-addair"  >PNR 추가 <span class="glyphicon glyphicon-plus" aria-hidden="true" ></span></button>&nbsp;<button type="button" class="btn btn-default btn-xs js-addairstop"  >STOP 추가 <span class="glyphicon glyphicon-plus" aria-hidden="true" ></span></button>&nbsp;
											...
											<tr class="airin2">
											  <td colspan='16'>
												  <table  id="airsub" class="table-bordered table-condensed gridSixteen airsub">
													<tbody>
														 <tr >
															<td colspan="16" class="active text-center formHeader"><span id="innum">항공 인보이스#<?=$i?></span></td>
															
														</tr>
														 <tr>
														    <td  class="active text-center formHeader" colspan=2>항공정보</td>
														 ...
														 <tr>
															...
															<td  class="active text-center formHeader">출발/도착시간</td>
															 ...
															 <input type="text" name="rdairtime[]" class="inpubase sm1" placeholder="도착시간" value="<?=$rrow[a_airport_time3]?>"/>
															</td>
															<td colspan="2"   class="active text-center formHeader">PNR/TICKET#</td>
															 <td  colspan="2">  <input type="text" name="rpnrnum[]" class="inpubase sm1" placeholder="pnr" value="<?=$rrow[a_pnr_number1]?>"/>
														...
														<tr>
															...
															<td  colspan="2" class="active text-center formHeader">Class</td>
															 <td colspan="2"> <select name=a_cls_type[] class=form-control>
														...
														<tr>
															...
															<td  class="active text-center formHeader">항공수익</td>
															...
															<td colspan="4"> <textarea class="form-control" rows="6" name="airetc[]"  placeholder="기타메모"><?=$rrow[memo_air]?></textarea>
														...
														<tr class="airstop" >
                                                          ...
                                                          ?>
																	   ...
																	   <table id="stopair" class="table table-bordered table-condensed gridSixteen reserveTable formDetail stopair" style="display:none;">
																		<tbody>
																			<tr >
																				<td rowspan="2" class="active text-center formHeader">STOP<br /><b>*PNR과 TK넘버는 반드시 <br /> 같아야합니다!!</b></td>
																				<td class="active text-center formHeader">  
												  ...
												  <table id="airsub" class="table-bordered table-condensed gridSixteen airsub">
													<tbody>
														 ...
														 <tr>
															...
															<td  class="active text-center formHeader">도착공항</td>
															 <td>  <input type="text" name="rde_air[]" class="inpubase sm1" aria-label="출발공항" value=""/>
															</td>
															<td  class="active text-center formHeader">편명</td>
															...
															<td  class="active text-center formHeader">출발/도착시간</td>
															 ...
															 <input type="text" name="rdairtime[]" class="inpubase sm1" placeholder="도착시간" value=""/>
															</td>
															<td colspan="2"  class="active text-center formHeader">PNR/TICKET#</td>
															 <td colspan="2" >  <input type="text" name="rpnrnum[]" class="inpubase sm1" placeholder="pnr" value=""/>
														...
														<tr>
															...
															<td colspan="4"> <textarea class="form-control" rows="6" name="airetc[]"  placeholder="기타메모"></textarea>
					...
					<tbody id="sbody">
						...
						<?php
									...
									if ($cntr > 0) {
										while($rrow = $rstr->fetch_assoc()):
										   $productInfo = getProductMaster($rrow[p_code]);
										   $picnm= pickBaseCodetxt($rrow[meet_area]);
											...
											<td colspan="3" class="no-right-border">
												<div class="row">
													<div class="col-sm-12">
														<!-- <input type="date" id="" name="startDate" class="inpubase" value=""/> -->
														<div class="input-group input-group-sm">
															<input type="date" name="singleDayTourStartDate[]" class="form-control js-singleDayTourDate" aria-label="출발일" placeholder="출발일" value=''>
															<span class="input-group-btn">
																<button class="btn btn-default popp" data-toggle="modal" data-target=".js-openSingleDayTourModal"  type="button" ><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span></button>
																	
															</span>
															<input type=hidden name='tday[]' class=form_box value="">
															<input type=hidden name='pos[]' class=form_box value="">
															<input type=hidden name='seqnum[]' class=form_box value="">
														</div>
													</div>
												</div>
											</td>
													...
													<div class="col-sm-12">
														...
														<div class="row">
															<div class="col-sm-12">
																	...
																	<span></span>
																	<input type=hidden name='l_p_code[]'  id="l_p_code" class=form_box value="">
																	<input type=hidden name='singleTour[]'  id="singleTour" class=form_box value=''>
			...
			<!--	<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
						...
						<tr>
							...
							<td colspan="2">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="arrivalDate" class="form-control js-dateInput js-arrivalDate" aria-label="도착일" placeholder="도착일" value="<?=$reserve_pick[air_arriveDate]?>">
											<span class="input-group-btn">
												<button class="btn btn-default js-dateInputBtn" type="button"><span class="glyphicon glyphicon-calendar" aria-hidden="true"></span></button>
											</span>
										</div>
							...
							<td colspan="1">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="arrivalTime" class="form-control " aria-label="도착시간" placeholder="도착시간" value="<?=$reserve_pick[air_arrivetime]?>">
											<!--<span class="input-group-btn">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											...
											<input type="text" name="airname" class="form-control" aria-label="항공사/편명" value="<?=$reserve_pick[air_arriveNm]?>"/>
										</div>
									</div>
								</div>
							</td>
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											...
											<input type="text" name="arrivememo" class="form-control" aria-label="메모" value="<?=$reserve_pick[air_arriveMemo]?>"/>
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
							...
							<td colspan="2">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="departureDate" class="form-control js-dateInput js-departureDate" aria-label="출발일" placeholder="출발일" value="<?=$reserve_send[air_stdate]?>">
											<span class="input-group-btn">
												<button class="btn btn-default js-dateInputBtn" type="button"><span class="glyphicon glyphicon-calendar" aria-hidden="true"></span></button>
											</span>
										</div>
							...
							<td colspan="1">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="departureTime" class="form-control" aria-label="출발시간" placeholder="출발시간" value="<?=$reserve_send[air_sttime]?>">
							...
							<td colspan="3">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<span class="input-group-addon">항공사/편명</span>
											<input type="text" name="departureairname" class="form-control" aria-label="항공사/편명" value="<?=$reserve_send[air_stNm]?>"/>
										</div>
									</div>
								</div>
							</td>
							<td colspan="9">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<span class="input-group-addon">메모</span>
											<input type="text" name="departurememo" class="form-control" aria-label="메모" value="<?=$reserve_send[air_stMemo]?>"/>
										</div>
									</div>
								</div>
							</td>
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">총옵션금액</td>
							<td colspan="14"><span id="totoamt"><?=$oamt1?></span><input type="hidden" name="otamt" id="otamt" value="<?= $totoptamt ?>"></td>
						</tr>
						<tr>
								...
								<div class="row no-nav">
									<div class="col-sm-12">
										<ul class="pagination non-nav flex-justify-flex-end">
											...
											<li <?php if ($reserve_info[payment_st]=='DONE') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>class="disabled"><span>완납
											<li <?php if ($reserve_info[payment_st]=='OPAY') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>class="disabled"><span>환불
										</ul>
									</div>
								</div>
							</td>
				...
				<div class="row">
					<div class="col-sm-6 col-sm-offset-6 text-right">
					  ...
					  <?php } else if (($reserve_info[settle_report] == '1') || ($reserve_info[settle_report] == '2')) { ?>
								...
								<button type="button" class="btn btn-xs btn-default js-done disabled" disabled  class="btn btn-xs btn-default js-ccr" onClick="go_corder()">예약확정</button>
								<button type="button" class="btn btn-xs btn-default js-can" disabled onClick="go_cancel()">예약취소</button>
								<button type="button" class="btn btn-xs btn-default js-prn">영수증 출력</button>
								<?php if ($prodInfo[p_type] == 5) { ?>
												...
												<?php
												 ...
												 ?>
																		<tr>
																			...
																			<td colspan="1" class="text-center"><?=$p_row[payment_status]?></td>
																			<td colspan="1" class="text-center"><?=$uinfo[kor_name]?></td>
																			<?php if ($user_dbinfo[userid] == 'sylvialee' || $user_dbinfo[userid] == 'wincom00') { ?>
    ...
    <script>
		$(document).ready(function () {
			...
			$('.printclass').datepicker({
				format: "yyyy-mm-dd",
				autoclose: true
			});
			$('.returnclass').datepicker({
				format: "yyyy-mm-dd",
				autoclose: true
			});
			*/
			...
			var editor = CKEDITOR.replace( "pmemo", {
				filebrowserUploadUrl: "./upload.php",
			    filebrowserUploadMethod: 'form',
			    removePlugins: 'easyimage,image2,divarea,elementspath',
			    extraPlugins: 'uploadimage,image',  // 기본 <img> 업로드/삽입만
				allowedContent : true,enterMode:"2",height : "305px",
			...
			pt.initReservationDetail()
			{
				...
				for (var i = 0; i < scope.length; i++) {
					...
					$(document).on('blur', '.js-opt', function(e){ 	
						 calc(1);
					});
					...
					process.off('click').on('click', function (e) {
						...
						var lastamton = $("#lastbalance").val()
						$('input[name^="lastpayamt"]').val(lastamton)
						...
						paymentTypeSelection.off('change').on('change', function (e) {
							...
							if (paymentType) {
								if ((paymentType === 'creditcard') || (paymentType === 'creditcardusa')) {
			...
			$(".js-optLoc").chosen();
			var i = parseInt('<?= isset($cnt12) ? (int)$cnt12 : 0 ?>') || 0;
			$(document).on('click', '.js-addOpt', function(e){ 
					...
					sHtml="<div class='form-inline js-optSet'>"+
							  "  	<select class='form-control  js-optLoc' style='width:200px;' name='optLoc"+currentIndex+"[]'  ID='optLoc"+currentIndex+"' multiple='multiple' data-live-search='true'>"+
							"		<?=optBaseCode4('',$pcode,'')?>"+
							"		</select>"+
							"		<input type='text'  name='optamt[]' id='optamt' class='form-control text-right js-opt' value=''	/>"+
							"		<select class='form-control js-optamt' name='optnm[]' id='optnm'>"+
							"		   <option value=''>선택이름</option>"+
							"		  <?php echo getReserveTrIn2($reserve_info[reserveCode],$opt_row['tnm']); ?>"+
							"		</select>"+
			...
			//$( ".js-removeopt" ).click(function() {
			$(document).on('click', '.js-removeopt', function(){ 
				    var btn = $(this)
					var actionActive = !btn.hasClass('hidden')
					var set = btn.closest('.js-optSet')
					if (actionActive) {
						set.remove()
					}
			...
			$(".js-optLoctemp").chosen();
			$(document).on('change', '.js-optLoctemp', function(){ 
			//'.js-optLoc').last().on('change', function() {
				
				$.ajaxSetup({async: false});
				var code1 = $(this).val();
			    var par = $(this).next().next(); 
			    var selValues = $(this).val();
				var dataval=0;
                //alert(selValues);
				$.getJSON("get_op.php?code1="+selValues, function(jsonData){
					 
					
					 $.each(jsonData, function(i,data){
						  
						  dataval=parseFloat(dataval)+parseFloat(data.opt_price);
            ...
            $(document).on('click', '.js-batch', function(e){
					$.ajaxSetup({async: false});
				    var $travelerTableBody = $('#travelerTableBody');
					var $optionsContainer = $('#optLoctemp');
					var $divopset =$('.js-optSet');
					var $tmat = $("#optamttemp");
				    var travelerNames = [];
					var amt = $("#optamttemp").val();
					var i=0;
					$divopset.each(function() {
						$(this).remove();
					});
					$travelerTableBody.find('tr').each(function() {
						// 테이블 내 input에서 이름 가져오기
						var name = $(this).find('#t_name').val()?.trim();
                        //alert(name);
						let selectedValues = $optionsContainer.val();
						let optcodes = []; // 최종 옵션 코드 배열 초기화
						if (selectedValues === null) {
							// 아무것도 선택되지 않은 경우 빈 배열
							optcodes = [];
						} else if (Array.isArray(selectedValues)) {
							// 다중 선택(<select multiple>)인 경우, 반환된 배열 그대로 사용
							optcodes = selectedValues;
						} else {
							// 단일 선택인 경우, 반환된 문자열을 배열에 넣음
							optcodes = [selectedValues];
						}
						...
						if (name) {
							...
							const currentIndex = i;
							$.ajax({
								url: 'get_op2.php', // *** 방금 만든 PHP 파일 경로 ***
								type: 'GET',
								dataType: 'html', // PHP가 HTML 문자열을 반환하므로 'html'로 설정
								data: {
									pcode: '<?=$pcode?>',
									code: optcodes // PHP의 $_POST['code']로 전달됨
									// 필요하다면 다른 데이터도 추가 가능 (예: travelerName, index)
								},
								success: function(optionsHtmlResponse) {
									// AJAX 성공 시 실행
									// 1. 새로운 <select> 요소를 포함한 전체 HTML 구조 생성
									sHtml="<div class='form-inline js-optSet'>"+
									  "  	<select class='form-control  js-optLoc' style='width:200px;' name='optLoc"+currentIndex+"[]'  ID='optLoc"+currentIndex+"' multiple='multiple' data-live-search='true'>"+
									"		"+optionsHtmlResponse+""+
									"		</select>"+
									"		<input type='text'  name='optamt[]' id='optamt' class='form-control text-right js-opt' value='"+$tmat.val()+"'	/>"+
									"		<select class='form-control js-optamt' name='optnm[]' id='optnm'>"+
									"		   <option value=''>선택이름</option>"+
									...
									// 2. 생성된 HTML을 페이지에 추가
									$(".optsec").append(sHtml); // '.optsec' 컨테이너에 추가

									// 3. 새로 추가된 select 요소에 Chosen 플러그인 적용
									$(".js-optLoc").chosen();
									i++;
								...
								error: function(jqXHR, textStatus, errorThrown) {
									...
									alert('옵션 정보를 가져오는 중 오류가 발생했습니다: ' + jqXHR.responseText);
								}
							});
			...
			//$( ".js-optLoc" ).change(function() {
			$(document).on('change', '.js-optLoc', function(){ 
			//'.js-optLoc').last().on('change', function() {
			...
			$('body').on('click','.js-removecomp1',function() {
				if(confirm("삭제할까요?") == true){
					var randid = $(this).closest("tr").find("#tourcomp1").val();
					var randty ='<?php echo $tab ?>';
					//alert(randty);
					var rev = $("#estimateCode").val();
					
					//alert(reserveCode);
					$.getJSON("comp_reset1.php?randid="+randid+"&rev="+rev+"&randty="+randty, function(jsonData){
			...
			$('body').on('click','.js-removecomp2',function() {
				if(confirm("삭제할까요?") == true){
					var randid = $("#tourcm").val();
					var rev = $("#estimateCode").val();
					
					//alert(reserveCode);
					$.getJSON("comp_reset2.php?randid="+randid+"&rev="+rev, function(jsonData){
			...
			$( ".js-dprocess" ).click(function() {
					...
					var seq = $(this).val();
					var bal = $("#tbalamt").val();
					$.getJSON("update_rdel2.php?reserveCode="+estimateCode+"&seq="+seq, function(jsonData){
						 $.each(jsonData, function(i,data){	
			...
			$(document).on('click', '.js-addairstop', function(){ 
				...
				$trNew = $trLast.clone().prop('style','display:block');
			...
			$(document).on('click', '.js-addRand', function(){ 
				
				var $tableBody = $('#addRandtab').find("#Rbody"),
				$trLast = $tableBody.find(".rand1:last"),
				$trNew = $trLast.clone(true);
				
				$trLast.after($trNew);
				//$trLast.chosen();
				//$(".comp2").trigger("liszt:updated");
			...
			if ('<?=$reserve_info[p_cnt]?>' == "")
			{
				var inint=0;
			} else {
				var inint='<?=$reserve_info[p_cnt]?>' - 1;

			}
			$(document).on('click', '.js-addTraveler1', function(){ 	
				
				var clonedRow = $('.innerTable').find('tr:last').clone();
				var innm = $('.innerTable').find('tr:first-child').find('#t_name').val();
				/*
				...
				*/
				inint++;
				var innm = innm + inint;
				clonedRow.find("td:eq(1)").find("input").val(innm);
			...
			$(document).on('click', '.js-rTraveler1', function(){ 	
				
				inint--;
				var numTouristsElem = $('.js-numTourists')
				var numTourists = parseInt(numTouristsElem.val())
				if (numTourists > 1) {
					var clonedRow = $('.innerTable').find('tr:last').remove();
					numTouristsElem.val(parseInt(numTourists) - 1)
				} else {
					numTouristsElem.val(2).prop({ readonly: true })
				}
			...
			$(document).on('click', '.js-copy', function(){ 	
				...
				var targetnm = $("#r_name").val();
				var targetph = $("#r_phone").val();
				var targetmail = $("#r_email").val();
				$('.innerTable').find('tr:last').find('#t_name').val(targetnm);
				$('.innerTable').find('tr:last').find('#t_phone').val(targetph);
				$('.innerTable').find('tr:last').find('#t_email').val(targetmail);
			...
			$(document).on('click', '.js-addroom1', function(e){ 
				var numRoomElem = $('.rcnt1')
				
                numRoomElem.val(parseFloat(numRoomElem.val()) + 0.5);

			});

			$(document).on('click', '.js-rRoom1', function(e){ 
				var numRoomElem = $('.rcnt1')
				var	numRoom = numRoomElem.val()
				if (numRoom >=0.5) {
					
					numRoomElem.val(parseFloat(numRoomElem.val()) - 0.5);
				} else {
					numRoomElem.val(0).prop({ readonly: true })
				}
		...
		function calc(gu) {
			...
			$('input[name^="optamt"]').each(function() {
			   optamt = optamt + parseFloat($(this).val());
			...
			var tttotamt = (unitamt+addamt+parseFloat(airtot)+optamt)-disamt;
			$("#totoamt").html('<?=$sign?> '+ optamt.toFixed(2));
			//alert(tttotamt);
			...
			$.getJSON("get_bal.php?code1="+code1+"&lastval="+disamt+"&lastval2="+addamt+"&unitamt="+unitamt+"&airamt="+airtot+"&optamt="+optamt	, function(jsonData){
	 
				 $.each(jsonData, function(i,data){
					  var unittamt  =parseFloat($("#ttamt").val());
					  var alltot = parseFloat($("#tgtotamt").val());
					  var baltmp = parseFloat(data.last_bal);
					  var lastdiff = alltot.toFixed(2) - baltmp.toFixed(2);
					  //alert(alltot);
					  //lert(baltmp);
					  //alert(lastdiff);
		...
		function go_pay() {
				  ...
				  if (($("#paymentmethod option:selected").val() == 'creditcard') || ($("#paymentmethod option:selected").val() == 'creditcardusa')) {
		...
		function openwin3(r_code) { 
	       var winName = "all_"+(ctr++);
		   window.open("invoice_page3.php?division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&r_code="+r_code,winName,"width=1000,height=700,scrollbars=1");
	    }
