...
<?php
    ...
    if ($mode == "save") {
		...
		$estimateCode = $_GET['estimateCode'];
		if ($estimateCode == "") {
		       // 토탈예약용 예약코드
				...
				$total_estimateCode = "THEL".date("His", $sec) . $hs;
				//$total_estimateCode = "THEL".date("his");	
    
				$estimateNum = getNumReserve();
				$estimateCode = "HELLO".date("His", $sec) . $hs;
			   ...
			   /*옵션정보*/
				$k=0;
				//echo count($optnm);
				for ($s=0; $s < count($optnm) ;$s++ ) {
					///print_r($optLoc1);
					
				
					for ($i=0; $i < count(${'optLoc'.$s}) ;$i++ ) {
                      $qryr1 = "select opt_price from base_opt where opt_code = '".${'optLoc'.$s}[$i]."'";
					  $rstr1 = $dbConn->query($qryr1);

					  $rowr1 = $rstr1->fetch_assoc();
					//  echo $qryr1;
						 
					  $totopt = $totopt  + $rowr1[opt_price];
					  if ($optnm[$s] !='') {					  
						  $qry3 = "INSERT INTO reserve_opt (
			   ...
			   for($j=0; $j<count($singleDayTourStartDate); $j++)
               {
				   ...
				   $picnm= pickBaseCodetxt($mtarea[$j]);
				   $addmemop .= $local_start."/".$singleTour[$j]."/".$picnm['pick_name']."\n";
			  ...
			  $pmemo =$pmemo.$addmemop;
			  $qry1 ="update reserve_info 
								set progress='$pmemo' where reserveCode='$estimateCode'";
              $rst1 = $dbConn->query($qry1);
			   ...
			   if ($pricet == 1) {
				...
				if ($order_status == "CANCEL") {
                    $qry1 = "delete from tour_car where reserveCode = '$estimateCode'";
					$rst = $dbConn->query($qry1);
			...
			//echo $qry1;
			   ...
			   $qryd = "delete from reserve_info 
										where
										reserveCode = '$estimateCode'  && parent = 'SUB' && (p_code!='$sendcode' && p_code!='$pickcode' ) ";
				//echo $qryd;
				//exit;
			   $rstd = $dbConn->query($qryd);
			   for($j=0; $j<count($singleDayTourStartDate); $j++)
