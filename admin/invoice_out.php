<?php
	...
	if ($mode == "print") {
		echo "<meta http-equiv='refresh' content='0; url=./invoice_p3.php?division=3&pdx=2&sub=15&r_code=".$r_code."'>";
...
<form name=print id=print action='' method=post enctype="multipart/form-data">
  ...
  <input type=hidden name=order_status id=order_status value="">	
	...
	<div id="invoice">
		...
		<div class="invoice1 overflow-auto"  >
			<div style="min-width: 600px">
				...
				<header>
					<div class="text-center confim_book">투어정보</div>
						
				</header>
				<br />
					<div id="custom_button" class="col-sm-12 text-right">
					    <?php if ($revInfo[settle_report] == 1) { ?>						   
						<button type="button" class="btn btn-xs btn-default js-report">정산확정</button>
					
						<button type="button" class="btn btn-xs btn-default js-cancel">정산취소</button>

						<?php } else if ($revInfo[settle_report] == 2) {  ?>
						<button type="button" class="btn btn-xs btn-default js-cancel">정산취소</button>
                        <?php } ?>
						<button type="button" class="btn btn-xs btn-default js-print" onclick="pageprint()">정산인쇄</button><br /><br />
				...
				<div class="row">
					<div class="col-sm-12">
						<table style='width: 100%;line-height: 18px;text-align: left;border: 1px solid #222;font-size: 13px;'>
							<tbody>
								<tr style="background: #eee;font-weight: bold;text-align: center;padding: 10px;border: 1px solid #222;">
									<th style="padding: 10px;border: 1px solid #222;" width='5%'>NO.</th>
									<th style="border: 1px solid #222;" width='10%'>성명</th>
									<th style="border: 1px solid #222;" width='5%'>성별</th>
									<th style="border: 1px solid #222;" width='10%'>DOB</th>
									<th style="border: 1px solid #222;" width='10%'>여권만료일</th>
									<th style="border: 1px solid #222;" width='10%'>여권번호</th>
									<th style="border: 1px solid #222;" >RM</th>
									<th style="border: 1px solid #222;" >연락처</th>
									<th style="border: 1px solid #222;"width='*'>특이사항</th>
								</tr>
				...
				<div class="row">
					...
					<div class="col-sm-12" >
						<?php
									...
									if ($cntr > 0) {
										while($rrow = $rstr->fetch_assoc()):
										   ...
										   if ($revInfo[re_issue]== 'Y') {
											  $recap = '<font color=red> RE-ISSUE </font>';
						...
						?>
							               <table style='width: 90%;line-height: 18px;text-align: left;border: 1px solid #222;font-size: 13px;'>
												<tbody>
													<tr style="background: #eee;font-weight: bold;text-align: center;padding: 5px;border: 1px solid #222;">
														<td colspan="15" width = '10%' style="border: 1px solid #222;padding: 5px;"><?=$recap?>&nbsp;&nbsp;항공 인보이스#<?=$i?></td>
													...
													<tr style="background: #eee;font-weight: bold;text-align: center;padding: 5px;border: 1px solid #222;">
														...
														<td colspan=4 style="background: #fff;padding: 5px;text-align: left;">&nbsp;<?=$rrow[a_pnr_number]?>/<?=$rrow[a_tk_number]?><input type=hidden name='a_tk_number[]' id=a_tk_number value="<?=$rrow[a_tk_number]?>"></td>
													</tr >
													<tr style="background: #eee;font-weight: bold;text-align: center;padding: 5px;border: 1px solid #222;">
	...
	<hr style='border-bottom: 2px solid #00000; !important'>
	<div id="invoice1" style="page-break-after: always;">
				...
				<main>
				   <div class="row contacts">
					...
					<table style='width: 100%;line-height: inherit;text-align: left;border: 1px solid #222;font-size: 13px;'>
						...
						?>
							<tr style="background: #fff;font-weight: 400;text-align: center;">
								...
								<td style='text-align: center;border: 1px solid #222;' width="20%"><?=$row[payment_status]?>(<?=$SC?>)</td>
					...
					<?php
						 ...
						 $totprofit = ($totamt +$agecamt[amt]) - $tot_expense;

						 //echo $tot_expense."|".$airprofit[pamt]."|".$cc_pmt_fee;
					...
					<div style="margin-top: 15px;padding-left: 0px !important;">
						...
						<div class="row tour-details">
						<div class="col-md-12 invoice-to">
							<h2 class="invoice-to">기타커미션수익</h2>
						</div>
					</div>
					<div style="margin-top: 15px;padding-left: 0px !important;">
						   ...
						   <table style='width: 100%;line-height: inherit;text-align: left;border: 1px solid #222;font-size: 13px;'>
								<thead>
									 <tr style="background: #eee;font-weight: bold;text-align: center;padding: 10px;border: 1px solid #222;">
										...
										<th width="15%" style="border: 1px solid #222;">커미션</th>
								...
								<tbody>
									<tr style="background: #fff;font-weight: 400;text-align: center;">
										<td style='text-align: left;border: 1px solid #222;padding: 5px;'><b><?=$sign?>  <?php echo number_format($agecamt[amt],2);?></b></td>
						   ...
						   <table style='width: 100%;line-height: inherit;text-align: left;border: 1px solid #222;font-size: 13px;'>
								...
								<tbody>
									<tr style="background: #fff;font-weight: 400;text-align: center;">
										...
										<td width="20%" style='text-align: center;border: 1px solid #222;color: red !important;'><b><?=$sign?>  <?php echo number_format($totprofit,2);?></b> <input type=hidden name=pay_profit_amt id=pay_profit_amt value="<?=$totprofit?>"></td>
...
<script>
	    ...
	    $(document).ready(function () {
			...
			$('body').on('click', '.js-report', function () {
				if(confirm("정산확정 하시겠습니까?") == true)
				{
					r_code =$("#r_code").val();
					$("#order_status").val('confirm');
					$("#print").attr("action","out_settle_process.php?division=6&pdx=3&sub=17&r_code="+r_code+"");
			...
			$('body').on('click', '.js-cancel', function () {
				if(confirm("정산취소 하시겠습니까?\n취소하면 정산페이먼트내역도 함꼐 초기화됩니다.") == true)
				{
					r_code =$("#r_code").val();
					$("#print").attr("action","out_settle_process.php?division=6&pdx=3&sub=17&r_code="+r_code+"");
		...
		{ 
		   ...
		   $("#print").attr("action","invoice_p3.php?division=6&pdx=3&sub=17&r_code="+r_code+"");
		   $("#print").submit();
		} 
