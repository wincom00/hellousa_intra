		  ...
		  if ($no == "") {
			 ...
			 // print_r($_POST["menu2"]);
			  ...
			  if(isset($_POST["menu2"])) {
					...
					while(list($key,$val)=each($_POST["menu2"])) {
						...
						<table class="table table-striped table-bordered table-condensed">
						    <tbody>
									...
									<tr bgcolor=#f9f9f9 height=28>
										...
										<td width=35%>&nbsp;<? if($v_info[userfile1]): ?><img src="product_img/thum_<?= $v_info[userfile1] ?>" ><? endif; ?> &nbsp;<input type=checkbox name=photo_del1 value="1"> Delete Image file</td>
									...
									<tr bgcolor=#f9f9f9 height=28>
										...
										<td width=85% colspan=3>&nbsp;<select name=guide_status class="inpubase md">
										<option value="GOOD" <? if($v_info[guide_status] == "GOOD") echo "selected"; ?>>근무가능
										<option value="DISABLE1" <? if($v_info[guide_status] == "DISABLE1") echo "selected"; ?>>근무불가(휴가)
										<option value="DISABLE2" <? if($v_info[guide_status] == "DISABLE2") echo "selected"; ?>>근무불가(병가)
										<option value="DISABLE3" <? if($v_info[guide_status] == "DISABLE3") echo "selected"; ?>>근무불가(개인사정)
										<option value="DISABLE4" <? if($v_info[guide_status] == "DISABLE4") echo "selected"; ?>>근무불가(징계)
										<option value="DISABLE5" <? if($v_info[guide_status] == "DISABLE5") echo "selected"; ?>>퇴사
										</select></td>
									</tr>
									<tr>
										<td colspan=4 height=35 bgcolor=#FFFFFF align=center><input type=submit value="정보 수정" class="btn btn-primary btn-sm"></td>
									</tr>
							</tbody>
						</table>
						<table class="table table-striped table-bordered table-condensed">
						    <tbody>
							       <tr>
									...
									<tr>
										...
										<td width=25% align=left bgcolor=#FFFFFF>
											&nbsp;<input type="radio" name="dept1" value = "A" <? if($v_info[dept_prior] == "A") echo "checked"; ?>> 활성 &nbsp;&nbsp;
										  <input type="radio" name="dept1" value = "J" <? if($v_info[dept_prior] == "J") echo "checked"; ?>> 비활성
										</td>
									</tr>
							</tbody>
						</table>
	...
	<script>
			...
			$("#stday").inputmask("9999-99-99",{placeholder:"____-__-__"});
              ...
              function chk(tf){
					  ...
					  selectAll(document.base_guide.elements['menu2[]']);
					  return true;
