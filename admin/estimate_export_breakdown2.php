<?php
// estimate_export_breakdown2.php (PHP 7.0 + mysqli)
...
ob_start();
include 'include/header.php'; // $dbConn: mysqli 연결 (object 또는 link)
ob_end_clean();

// --- mysqli 연결 수비수 (object/리소스 모두 허용) ---
if (!isset($dbConn)) {
  die('DB connection ($dbConn) is not defined.');
}
if ($dbConn instanceof mysqli) {
  $mysqli = $dbConn;
} else {
  // 일부 환경에서 procedural 리소스로 넘어오는 경우
  $mysqli = $dbConn;
}
...
function rs_fetch_all_assoc($res){
  $rows = [];
  if($res){
...
// 데이터 로딩
$master = [];
$r = q("SELECT * FROM estimate_master WHERE id=".$id);
if($r && mysqli_num_rows($r)) $master=mysqli_fetch_assoc($r);

$items = [];
$r = q("SELECT * FROM estimate_items WHERE estimate_id=".$id." ORDER BY section,id");
if($r){ $items = rs_fetch_all_assoc($r); }
...
function build_dates($master,$pool=[]){
  $dates=[];
  ...
  if($sd!=='' && $ed!=='' && strtotime($sd)!==false && strtotime($ed)!==false){
    ...
    $set=[]; foreach($pool as $d) $set[$d]=true; $dates=array_keys($set); sort($dates);
  }
...
<head>
<meta charset="utf-8">
<title>BREAKDOWN QUOTATION</title>
<style>
...
<div class="wrap">
  ...
  <div class="header">
    <h1 class="title">BREAKDOWN QUOTATION<span class="badge">PRINT</span></h1>
    <div class="meta">
      작성일: <?= htmlspecialchars(isset($master['wdate'])?$master['wdate']:'', ENT_QUOTES, 'UTF-8') ?><br>
      출력일: <?= date('Y-m-d H:i') ?>
    </div>
  </div>
  ...
  <table class="tbl">
    ...
    <tbody>
      <?php foreach($sections['HOTEL'] as $it):
        $etc=json_decode(isset($it['etc_json'])?$it['etc_json']:'{}',true); if(!is_array($etc)) $etc=[];
  ...
  if(!empty($sections['MEAL'])):
    ...
    foreach($sections['MEAL'] as $it){
      $e=json_decode(isset($it['etc_json'])?$it['etc_json']:'{}',true); if(!is_array($e)) $e=[];
    ...
    $tot=0.0; $row=[]; $perUnit=0; $paxShow=0;
    foreach($types as $t){
  ...
  if(!empty($sections['TRANSPORT'])):
    ...
    $dates=build_dates($master,$pool);
    $rows=[]; // label => ['unit'=>0,'veh'=>0,'m'=>[d=>cnt]]
    foreach($sections['TRANSPORT'] as $it){
      ...
      if(!isset($rows[$label])){ $rows[$label]=['unit'=>0,'veh'=>0,'m'=>[]]; foreach($dates as $d) $rows[$label]['m'][$d]=0; }
  ...
  <?php $section_totals['TRANSPORT']=$tot; endif; ?>
	...
	if(!empty($sections['OVERTIME'])):
	  ...
	  // 2) 타겟(대상)별로 집계
	  //    - cnt: 대상 수 (etc.unit_per_target 누적, 없으면 item.cnt)
	  //    - m[date]: 해당 날짜의 시간/단위 수(합계). 리스트 형태면 1씩 증가
	  $rows=[]; // key => ['cnt'=>0,'m'=>[d=>float]]
	  foreach($sections['OVERTIME'] as $it){
		$label = trim((string)(isset($it['label'])?$it['label']:''));
		$e = json_decode(isset($it['etc_json'])?$it['etc_json']:'{}',true); if(!is_array($e)) $e=[];
		$target = trim((string)($e['target'] ?? $label ?: 'Overtime'));
		if($target==='') $target = 'Overtime';

		if(!isset($rows[$target])){
		  $rows[$target] = ['cnt'=>0.0,'m'=>[]];
		  foreach($dates as $d) $rows[$target]['m'][$d]=0.0;
		}

		// 대상 수(unit_per_target) 누적 (없으면 item.cnt 사용)
		if(isset($e['unit_per_target']) && is_numeric($e['unit_per_target'])){
		  $rows[$target]['cnt'] += (float)$e['unit_per_target'];
		} elseif(isset($it['cnt']) && is_numeric($it['cnt'])) {
		  $rows[$target]['cnt'] += (float)$it['cnt'];
		}

		// 날짜별 값 누적
		if(!empty($e['dates']) && is_array($e['dates'])){
		  if(is_assoc_array($e['dates'])){
			foreach($e['dates'] as $d=>$v){
			  if(isset($rows[$target]['m'][$d])) $rows[$target]['m'][$d] += (float)$v;
			}
		  }else{
			foreach($e['dates'] as $d){
			  if(isset($rows[$target]['m'][$d])) $rows[$target]['m'][$d] += 1.0;
			}
		  }
		}
	  }
	  ...
	  // 3) 합계 계산: Σ(dates) × 대상수
	  $tot=0.0; $rowTot=[];
	  foreach($rows as $nm=>$r){
		$occ = 0.0; foreach($dates as $d) $occ += (float)$r['m'][$d];
		$rowTot[$nm] = ((float)$r['cnt']) * $occ;
		$tot += $rowTot[$nm];
	  }
	?>
	<div class="hsec">3-1) OVERTIME</div>
	<table class="tbl">
	  <thead>
		<tr>
		  <th width="12%">대상</th>
	  ...
	  <tbody>
		...
		<tr>
		  ...
		  <?php foreach($dates as $d): ?>
			<td><?= (float)$r['m'][$d] == (int)$r['m'][$d] ? (int)$r['m'][$d] : number_format((float)$r['m'][$d],2) ?></td>
		  <?php endforeach; ?>
		  <td><?= number_format((float)$r['cnt'],2) ?></td>
		  <td class="right cur"><?= money($rowTot[$nm]) ?></td>
		</tr>
		<?php endforeach; ?>
		<tr class="sumrow">
		  <td class="right" colspan="<?= 1 + count($dates) ?>">OVERTIME 소계</td>
		  <td></td>
		  <td class="right cur"><?= money($tot) ?></td>
		</tr>
	  </tbody>
	</table>
	<?php $section_totals['OVERTIME']=$tot; endif; ?>
