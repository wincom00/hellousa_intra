        ...
        if ($xlsx) {
            ...
            foreach ($sheet_names as $sheet_index => $sheet_name) {
                try {
                    ...
                    // 그룹 정보 병합 전략: 첫 번째 유효한 정보를 기준으로, 나머지는 빈 필드 채우기
...
function saveGroupReservation($group_data, $travelers_data, $partner_id) {
    ...
    try {
        ...
        $grand_revNo = 'THEL' . date('ymdHis') . mt_rand(1, 9);
        $current_reserveCode = 'HELLO' . date('ymdHis') . mt_rand(1, 9);
        ...
        if ($resolved_product_code === '' || $resolved_tour_name === '') {
            $resolved_product = resolveProductInfoForSave($group_data);
            if (empty($resolved_product['success'])) {
                throw new Exception($resolved_product['message'] ?? 'DB 상품 조회 실패');
            }
...
<head>
    ...
    <style>
        ...
        .navbar {
            background: linear-gradient(135deg, #3993ba 0%, #3993ba 100%);
    ...
    <nav class="navbar navbar-expand-lg">
        <div class="container">
            <a class="navbar-brand text-white" href="index.php">
                <i class="fas fa-arrow-left me-2"></i> Hello USA - 스마트등록
            </a>
