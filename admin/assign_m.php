<?php
   ...
   // include "include/inc_base.php";
	...
	if (!function_exists('appendGuideLabelsFromValue')) {
		function appendGuideLabelsFromValue(&$labels, $labelValue)
		{
			...
			$parts = explode("+", $labelValue);
			foreach ($parts as $part) {
				appendGuideLabel($labels, $part);
			}
		}
	}
	...
	if (!function_exists('getGuideDisplayNameByMemberId')) {
		function getGuideDisplayNameByMemberId($memberId)
		{
			...
			if ($memberInfo['eng_name'] != "") {
				return $memberInfo['eng_name'];
			}
	...
	if (!function_exists('getGuideNameByTourMainGuideProduct')) {
		function getGuideNameByTourMainGuideProduct($grandECode, $stDate, $pCode, $subECode = "", $busNum = "")
		{
			...
			if ($safeBusNum != "") {
				$qry .= " and a.bus_num = '$safeBusNum'";
			}
			$qry .= "
					order by a.bus_num asc, a.seq_no asc";
           
			$rst = $dbConn->query($qry);
			if ((!$rst || $rst->num_rows == 0) && ($safeSubECode != "" || $safeBusNum != "")) {
	...
	if (!function_exists('getReservationMainGuideTargets')) {
		function getReservationMainGuideTargets($reserveCode)
		{
			global $dbConn;

			static $targetCache = array();

			$reserveCode = trim((string)$reserveCode);
			if ($reserveCode == "") {
				return array();
			}

			if (isset($targetCache[$reserveCode])) {
				return $targetCache[$reserveCode];
			}

			$safeReserveCode = $dbConn->real_escape_string($reserveCode);
			$qry = "select distinct
							a.p_code,
							a.p_name,
							a.stDate,
					...
					where a.reserveCode = '$safeReserveCode'
					  and a.parent = 'SUB'
			...
			if ($rst) {
				while ($row = $rst->fetch_assoc()) {
					$targetKey = implode("|", array(
						trim((string)$row['grand_eCode']),
						trim((string)$row['sub_eCode']),
						trim((string)$row['p_code']),
						trim((string)$row['stDate']),
						trim((string)$row['bus_num'])
					));
					if (isset($seenKeys[$targetKey])) {
						continue;
					}
			...
			$targetCache[$reserveCode] = $targets;
			return $targetCache[$reserveCode];
		}
	}
	if (!function_exists('getReservationMainGuideName')) {
		function getReservationMainGuideName($grandECode, $subECode, $pCode, $stDate, $busNum = "")
		{
			...
			$mainGuideLabels = array();
			$busFilter = ($safeBusNum != "") ? " and bus_num = '$safeBusNum'" : "";
			$subECodeFilter = ($safeSubECode != "") ? " and sub_eCode = '$safeSubECode'" : "";
			...
			$reserveQry = "select reserveCode
						   ...
						   where grand_eCode = '$safeGrandECode'
						     and p_code = '$safePCode'
						     and stDate = '$safeStDate'
						     and reserveCode != ''".$subECodeFilter.$busFilter."
						   order by sub_eNum+0 asc, reserveCode asc
						   limit 1";
			$reserveRst = $dbConn->query($reserveQry);
			if ((!$reserveRst || $reserveRst->num_rows == 0) && $safeSubECode != "") {
				$reserveQry = "select reserveCode
							   from tour_car
							   where grand_eCode = '$safeGrandECode'
							     and p_code = '$safePCode'
			...
			if ($reserveCode != "") {
				$mainGuideTargets = getReservationMainGuideTargets($reserveCode);
				foreach ($mainGuideTargets as $targetRow) {
					$mainGuideDate = trim((string)$targetRow['stDate']);
					if ($mainGuideDate == "") {
						$mainGuideDate = $stDate;
					}

					$mainGuideName = getGuideNameByTourMainGuideProduct(
						$targetRow['grand_eCode'],
						$mainGuideDate,
						$targetRow['p_code'],
						$targetRow['sub_eCode'],
						$targetRow['bus_num']
					);
					appendGuideLabelsFromValue($mainGuideLabels, $mainGuideName);
				}
			}

			if (!empty($mainGuideLabels)) {
				$mainGuideCache[$cacheKey] = implode("+", $mainGuideLabels);
				return $mainGuideCache[$cacheKey];
			}
			...
			if ($fallbackRst) {
				while ($fallbackRow = $fallbackRst->fetch_assoc()) {
					...
					appendGuideLabelsFromValue($fallbackGuideLabels, $mainGuideName);
				}
			}

			$mainGuideCache[$cacheKey] = !empty($fallbackGuideLabels)
				? implode("+", $fallbackGuideLabels)
				: "";
			return $mainGuideCache[$cacheKey];
		}
	}
	...
	if (!function_exists('getAssignedReservationCategoryName')) {
		function getAssignedReservationCategoryName($grandECode, $subECode, $pCode, $stDate, $busNum = "")
		{
			global $dbConn;
			...
			$subECodeFilter = ($safeSubECode != "") ? " and tc.sub_eCode = '$safeSubECode'" : "";
			$busFilter = ($safeBusNum != "") ? " and tc.bus_num = '$safeBusNum'" : "";

			$qry = "SELECT pm.p_cat_name
					FROM tour_car tc
					INNER JOIN reserve_info ri ON ri.reserveCode = tc.reserveCode
					INNER JOIN product_master pm ON ri.p_code = pm.p_code
					WHERE tc.grand_eCode = '$safeGrandECode'
					  AND tc.p_code = '$safePCode'
					  AND tc.stDate = '$safeStDate'
					  ".$subECodeFilter."
					  ".$busFilter."
					  AND ri.parent = 'MAIN'
					  AND ri.rev_status != 'CANCEL'
					  AND pm.p_cat_name != ''
					ORDER BY tc.sub_eNum+0 asc, tc.reserveCode asc, ri.seq_no asc
					LIMIT 1";

			$rst = $dbConn->query($qry);
			if ($rst && $rst->num_rows > 0) {
				$row = $rst->fetch_assoc();
				$categoryCache[$cacheKey] = trim($row['p_cat_name'] ?? '');
				return $categoryCache[$cacheKey];
			}
	...
	if (!function_exists('buildGuideAssignMemoHtml')) {
		function buildGuideAssignMemoHtml($stDate, $updatedAt, $isLatest = false)
		{
			...
			if ($rst) {
				while ($row = $rst->fetch_assoc()) {
					$guideName = getGuideDisplayNameByMemberId($row['guide_id']);

					$subGuideName = "";
					if ($row['sguide_id'] != "") {
						$subGuideName = getGuideDisplayNameByMemberId($row['sguide_id']);
					}
					...
					$parentCatName = getAssignedReservationCategoryName($row['grand_eCode'], $row['sub_eCode'], $row['p_code'], $row['stDate'], $row['bus_num']);
