...
if(isset($_GET['action']) && $_GET['action']==='export'){
    ...
    while($a=$res->fetch_assoc()){
        if($type==='hotel_mr'){
            ...
            $rows[] = [$a['월'], $label, $a['지역코드'], $a['호텔명'], $a['호텔코드'], (int)$a['객실수']];
            continue;
        }
        if($type==='monthly_region'){
            $label = codebase_name($dbConn,$a['지역코드']);
            $rows[] = [$a['월'], $label, $a['지역코드'], (int)$a['객실수']];
            continue;
        }
        if($type==='region'){
            $label = codebase_name($dbConn,$a['지역코드']);
            $rows[] = [$label, $a['지역코드'], (int)$a['객실수']];
            continue;
        }
        if($type==='weekly_region'){
            $label = codebase_name($dbConn,$a['지역코드']);
            $rows[] = [$a['주'], $label, $a['지역코드'], (int)$a['객실수']];
            continue;
        }
        $rows[]=array_values($a);
    }
...
foreach($region_all_raw as $r){
    ...
    $lab  = fmtRegionLabel_from_codebase('', $code, $dbConn);
    $region_all[] = ['region'=>$lab, 'code'=>$code, 'rooms'=>(int)$r['rooms']];
}
    ...
    <div class="tab-content" style="padding-top:12px;">
      ...
      <div id="tabMonthly" class="tab-pane fade in active">
        ...
        <table class="table table-bordered table-condensed js-sortable">
          ...
          <tbody>
          <?php $sum=0; foreach($monthly as $r): $sum+= (int)$r['rooms']; ?>
            <tr><td><?=htmlspecialchars($r['ym'])?></td><td class="text-right"><?=number_format($r['rooms'])?></td></tr>
          <?php endforeach; ?>
          </tbody>
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sum)?></th></tr></tfoot>
        </table>
      ...
      <div id="tabWeekly" class="tab-pane fade">
        ...
        <table class="table table-bordered table-condensed js-sortable">
          ...
          <tbody>
          <?php $sumw=0; foreach($weekly as $r): $sumw += (int)$r['rooms']; ?>
            <tr><td><?=htmlspecialchars($r['wk'])?></td><td class="text-right"><?=number_format($r['rooms'])?></td></tr>
          <?php endforeach; ?>
          </tbody>
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumw)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabYearly" class="tab-pane fade">
        ...
        <table class="table table-bordered table-condensed js-sortable">
          ...
          <tbody>
          <?php $sumy=0; foreach($yearly as $r): $sumy+=(int)$r['rooms']; ?>
            <tr><td><?=htmlspecialchars($r['yy'])?></td><td class="text-right"><?=number_format($r['rooms'])?></td></tr>
          <?php endforeach; ?>
          </tbody>
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumy)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabHotel" class="tab-pane fade">
        ...
        <table class="table table-bordered table-condensed js-sortable">
          ...
          <tbody>
          <?php $sumh=0; foreach($hotel as $r): $sumh+=(int)$r['rooms']; ?>
            <tr>
          ...
          <tfoot><tr><th colspan="2">합계</th><th class="text-right"><?=number_format($sumh)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabRegion" class="tab-pane fade">
        ...
        <table class="table table-bordered table-condensed js-sortable">
          ...
          <tbody>
          <?php $sumr=0; foreach($region_all as $r): $sumr += (int)$r['rooms']; ?>
            <tr>
              <td>
          ...
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumr)?></th></tr></tfoot>
        </table>
      </div>
        ...
        <div class="panel panel-default" style="margin-top:12px;">
          ...
          <div class="panel-body" style="padding:0;">
            <table class="table table-bordered table-condensed js-sortable" style="margin:0;">
              ...
              <tbody>
              <?php $sum_mr=0; foreach($hotel_mr as $r): $sum_mr+=(int)$r['rooms']; ?>
                <tr>
              ...
              <tfoot><tr><th colspan="4" class="text-right">합계</th><th class="text-right"><?=number_format($sum_mr)?></th></tr></tfoot>
            </table>
          </div>
        </div>
      </div>
