...
<?php
	...
	if($estimateCode)
	{
			...
			//exit;
			 ...
			 $html = file_get_html('http://www.scotiabank.com/rates/fxrates.html');
			 $num = 0;
			 $num1 = 0;
			 foreach($html->find('td[class="rate"]') as $e) {
					if ($num == 0) {
					   $title = $e->plaintext;
					}
					if ($num == 1) {
...
?>
	<div id="contentwrapper" class="reservationDetailForm">
		<div class="main_content">
			...
			<form action="<?= $PHP_SELF ?>?estimateCode=<?=$estimateCode?>&division=<?=$division?>&pdx=<?=$pdx?>&sub=<?=$sub?>&st=<?=$st?>&ty=<?=$ty?>&pcode=<?=$pcode?>&pricet=<?=$pricet?>" name="frmreserve" id="frmreserve" method="post" Enctype="multipart/form-data" >
				...
				<div class="row no-nav">
					<div class="col-sm-6">
						<ul class="pagination non-nav">
							...
							<li <?php if (($reserve_info[rev_status]=='ORDER') && ($revst != 'WAIT')) { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>예약확정</span></li>
							<li <?php if (($reserve_info[rev_status]=='DONE') && ($revst != 'WAIT')) { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>최종안내</span></li>
							<li <?php if (($reserve_info[rev_status]=='WAIT') && ($revst == 'WAIT')) { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>예약대기</span></li>
				...
				<div class="row no-nav">
					<div class="col-sm-6">
						<ul class="pagination non-nav">
							...
							<li <?php if ($reserve_info[payment_st]=='GPAY') { ?> class="active" <? } else { ?> class="disabled" <?php } ?>><span>보증</span></li>
					...
					<div class="col-sm-6 text-right">
					  <?php if ($estimateCode) { ?>
							<button type="button" class="btn btn-xs btn-default js-rr" onClick="go_submit()">예약수정저장</button>
							<button type="button" <?php if (((($reserve_info[rev_status]=='ORDER') || ($reserve_info[rev_status]=='CANCEL') || ($acnt > $prodInfo[p_cnt]) || ($reserve_info[rev_status]=='DONE')) && ($reserve_info[rev_status]!='')) || ($revst == 'WAIT')){ ?> class="btn btn-xs btn-default js-cr disabled" disabled <? } else { ?> class="btn btn-xs btn-default js-cr" <?php } ?> onClick="go_order()">예약확정</button>
							<button type="button" <?php if (((($reserve_info[rev_status]=='DONE') || ($reserve_info[rev_status]=='CANCEL') ||($acnt > $prodInfo[p_cnt])  ||
							($reserve_info[rev_status]=='READY')) && ($reserve_info[rev_status]!='')) || ($revst == 'WAIT')) { ?> class="btn btn-xs btn-default js-done disabled" disabled <? } else { ?> class="btn btn-xs btn-default js-ccr" <?php } ?> onClick="go_corder()">최종안내</button>
							<button type="button" class="btn btn-xs btn-default js-can" onClick="go_cancel()">예약취소</button>
							<button type="button" class="btn btn-xs btn-default js-prn" onClick="javascript:openwin('<?=$estimateCode?>')">고객 영수증 출력</button>
					  <?php } else { ?>
					        <button type="button" class="btn btn-xs btn-default js-rr" onClick="go_submit()">예약접수</button>
							<button type="button" class="btn btn-xs btn-default js-cr disabled" disabled class="btn btn-xs btn-default js-cr" onClick="go_order()">예약확정</button>
							<button type="button" class="btn btn-xs btn-default js-done disabled" disabled  class="btn btn-xs btn-default js-ccr" onClick="go_corder()">최종안내</button>
							<button type="button" class="btn btn-xs btn-default js-can" disabled onClick="go_cancel()">예약취소</button>
							<button type="button" class="btn btn-xs btn-default js-prn" disabled>고객 영수증 출력</button>
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail js-base">
					<tbody>
						...
						<tr>
							...
							<td colspan="2" class="active text-center formHeader">여행정원</td>
							<td colspan="2"><div  class="text-right pcntid"><?=$bcnt?>명</div></td>
						...
						<tr>
							...
							<td colspan="2">
								<div class="row">
									<div class="col-sm-12">
										<!--<div class="input-group input-group-sm">
											<input type="text" min="1" name='pcnt1' id='pcnt1' class="form-control text-right js-numTourists" aria-label="명" value="<?php if ($reserve_info[p_cnt]) {  echo $reserve_info[p_cnt]; } else { ?>1<?php } ?>"
							...
							<td colspan="2" class="active text-center formHeader">예약가능인원</td>
							<td colspan="2"><div class="text-right acntid"><?=$acnt?>명</div></td>
						</tr>
					</tbody>
				</table>
				<table id="example" class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						<tr>
							<td colspan="16" class="active text-center formHeader fullWidth">예약상세정보<font color=red> (기준통화 : <?=$prodInfo[base_rate]?>)</font></td>
						</tr>
						<tr class="innerTable">
							<td colspan="2" class="active text-center formHeader">예약자정보</td>
							<td colspan="3">
						...
						?>
									<tr class="innerTable">
										<td colspan="2" rowspan=<?=$cnt?> class="active text-center formHeader">여행자정보 &nbsp;<span id="ccnt"></span><button type="button" class="btn btn-default btn-xs js-addTraveler" ><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
										<td colspan="18" class="innerTable">
											<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail innerTable js-touristInfo">
												<tbody>
													<tr >
														...
														<td colspan="4">
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon">이름</span>
																		<input type="text" class="form-control inname" aria-label="이름" name="t_name[]" id="t_name" value="<?=$rowt[traveler_nm]?>"/>
																	</div>
																</div>
															</div>
														</td>
														<td colspan="4">
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon"><span class="glyphicon glyphicon-phone-alt" aria-hidden="true"></span></span>
																		<input type="text" class="form-control" aria-label="전화번호" name="t_phone[]" id="t_phone"
														...
														<td colspan="4">
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon">이메일</span>
																		<input type="text" class="form-control" aria-label="이메일" name="t_email[]" id="t_email" value="<?=$rowt[traveler_email]?>"/>
																	</div>
																</div>
															</div>
														</td>
													...
													<tr>
														...
														<!--<td colspan="2">
															<select class="form-control js-pickRoomType" name="pickRoomType[]" id="pickRoomType">
																<option value="" selected>- 룸타입 선택하세요 -</option>
														...
														<td colspan="2">
															...
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<input type="text" name="unitPrice[]" class="form-control text-right js-pickPrice" aria-label="판매가" placeholder="판매가" value="<?=$rowt[sale_price]?>" readonly/>
																		<span class="input-group-btn">
														...
														<td colspan="1">
															...
															<select class="form-control js-pickPriceType1" name="pickPriceType1[]" id="pickPriceType1" disabled>
														...
														<td colspan="2">
															...
															<select class="form-control js-pickRoomType1" name="pickRoomType1[]" id="pickRoomType1" disabled>
																<option value="" selected readOnly>- 룸타입 -</option>
														...
														<td colspan="2">
															...
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon">추가납부</span>
														...
														<td colspan="2">
															<!-- <input type="text" class="form-control" placeholder="할인"> -->
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon">할인</span>
														...
														<td colspan="2">
															<!-- <input type="text" class="form-control" placeholder="총액" readonly> -->
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
														...
														<td colspan="4">
															<div class="row">
																<div class="col-sm-6">
																	<button type="button" class="btn btn-xs btn-block btn-default js-resetTraveler">초기화</button>
																</div>
																<div class="col-sm-6">
																	<button type="button" class="btn btn-xs btn-block btn-default hidden  js-removeTraveler" >삭제</button>
																</div>
															</div>
														</td>
													</tr>
											...
											<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail innerTable js-touristInfo">
												<tbody>
													<tr>
														...
														<td colspan="4">
															<div class="row">
																<div class="col-sm-12">
																	<div class="input-group input-group-sm">
																		<span class="input-group-addon inname">이름</span>
																		<input type="text" class="form-control" aria-label="이름" name="t_name[]" id="t_name" value="<?=$rowt[traveler_nm]?>"/>
																	</div>
																</div>
															</div>
														</td>
													...
													<tr>
														...
														<td colspan="1">
														    <input type="hidden" class="form-control js-pickPriceType" name="pickPriceType[]" id="pickPriceType" value="<?=$rowt[pick_type]?>">
															<select class="form-control js-pickPriceType1" name="pickPriceType1[]" id="pickPriceType1" disabled>
																<option value="" selected>가격타입</option>
																<option <?php if ($rowt[pick_type] == "G") { ?> selected <?php } ?> value="G">성인</option>
																<option <?php if ($rowt[pick_type] == "I") { ?> selected <?php } ?> value="I">어린이</option>
														...
														<td colspan="2">
															...
															<select class="form-control js-pickRoomType1" name="pickRoomType1[]" id="pickRoomType1" disabled>
																<option value="" selected readOnly>룸타입</option>
																<option <?php if ($rowt[room_type] == "0r1p") { ?> selected <?php } ?> value="1r1p">당일</option>
																<option <?php if ($rowt[room_type] == "1r1p") { ?> selected <?php } ?> value="1r1p">1인1실</option>
																<option <?php if ($rowt[room_type] == "1r2p") { ?> selected <?php } ?> value="1r2p">2인1실</option>
																<option <?php if ($rowt[room_type] == "1r3p") { ?> selected <?php } ?> value="1r3p">3인1실</option>
																<option <?php if ($rowt[room_type] == "1r4p") { ?> selected <?php } ?> value="1r4p">4인1실</option>
														...
														<td colspan="4">
															<div class="row">
																...
																<div class="col-sm-6">
																	<button type="button" class="btn btn-xs btn-block btn-default  js-removeTraveler" >삭제</button>
																</div>
															</div>
														</td>
													</tr>
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">여행자정보 &nbsp;<button type="button" class="btn btn-default btn-xs js-addTraveler" ><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
							<td colspan="18" class="innerTable">
								<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail innerTable js-touristInfo">
									<tbody>
										<tr>
											...
											<td colspan="4">
												<div class="row">
													<div class="col-sm-12">
														<div class="input-group input-group-sm">
															<span class="input-group-addon inname">이름</span>
															<input type="text" class="form-control" aria-label="이름" name="t_name[]" id="t_name" value=""/>
														</div>
													</div>
												</div>
											</td>
											...
											<td colspan="4">
												<div class="row">
													<div class="col-sm-12">
														<div class="input-group input-group-sm">
															<span class="input-group-addon">이메일</span>
															<input type="text" class="form-control" aria-label="이메일" name="t_email[]" id="t_email" value=""/>
														</div>
													</div>
												</div>
											</td>
										...
										<tr>
											...
											<!--<td colspan="2">
												<select class="form-control js-pickRoomType" name="pickRoomType[]" id="pickRoomType">
													...
													<option value="1r2p">2인1실</option>
													<option value="1r3p">3인1실</option>
													<option value="1r4p" selected>4인1실</option>
											...
											<td colspan="2">
												...
												<div class="row">
													<div class="col-sm-12">
														<div class="input-group input-group-sm">
															<input type="text" name="unitPrice[]" class="form-control text-right js-pickPrice" aria-label="판매가" placeholder="판매가" value='<?=$pricead?>' readonly/>
															<span class="input-group-btn">
																<button class="btn btn-default dropdown-toggle tourPricePick-dropdown js-tourPricePick" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span></button>
																	<ul class="dropdown-menu js-tourPriceDropdown">
											...
											<td colspan="1">
											    <input type="hidden" name="pickPriceType[]" class="form-control js-pickPriceType" id="pickPriceType" value="G">
												<select class="form-control js-pickPriceType1" name="pickPriceType1[]" id="pickPriceType1" disabled>
													<option value=""  >가격타입</option>
													<option value="G" selected>성인</option>
													<option value="I">어린이</option>
													
												</select>							
											</td>
											<td colspan="2">
											    <input type="hidden" name="pickRoomType[]" id="pickRoomType" class="form-control js-pickRoomType" value="1r4p">
												<select class="form-control js-pickRoomType1" name="pickRoomType1[]" id="pickRoomType1" disabled>
											...
											<td colspan="2">
												...
												<div class="row">
													<div class="col-sm-12">
														<div class="input-group input-group-sm">
															<span class="input-group-addon">총액</span>
															<input type="text" name="lasttamt[]" id="lasttamt" class="form-control text-right js-totalPerPerson" aria-label="총액" value="<?=$pricead?>" readonly/>
														</div>
													</div>
												</div>
											</td>
						...
						?>
						<tr>
							<td colspan="2" class="active text-center formHeader">할인사유</td>
							<td colspan="3">
								<select class="form-control discount" name="pickloc">
									<option value="">- 할인사유코드 선택하세요 -
									<?=printBaseCode_first("S01",$reserve_info[dis_code])?>
								</select>							
							</td>
							<td colspan="11">
								<!-- <input type="text" class="form-control" placeholder="할인사유내용 (할인사유코드에 따라 영수증에 취소귀정바꿤)" title="할인사유코드에 따라 영수증에 취소귀정바꿤"> -->
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<span class="input-group-addon">취소규정</span>
											<!--<input type="text" name="dismemo" class="form-control" aria-label="할인사유내용" placeholder="할인사유코드에 따라 영수증에 취소규정바뀜" title="할인사유코드에 따라 영수증에 취소규정바뀜" value="<?=$reserve_info[dis_desc]?>"/>-->
											<select class="form-control" name="dismemo" id="dismemo">
												<option value="">- 선택 -
												<!--<?=printBaseCode_second("S01",$reserve_info[dis_code],$reserve_info[dismemo])?>-->
												<?=printBaseCode_first("C02",$reserve_info[c_code])?>
											</select>	
										</div>
									</div>
								</div>
						...
						<?php 
						  if (($pricet == '3') || (($prodInfo[p_type] == 5))) {
						?>
						<tr>
							<td colspan="2" class="active text-center formHeader">수금-협력사</td>
							<td colspan="4">
								<div class="row">
									<div class="col-sm-6">
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">지급-협력사</td>
							<td colspan="4">
								<div class="row">
									<div class="col-sm-6">
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">고객/협력사요청사항</td>
							<td colspan="14">
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						<tr>
							<td colspan="16" class="active text-center formHeader fullWidth">단일투어정보</td>
						</tr>
						<?php
									$qryr = "select a.* from reserve_info a  where  a.reserveCode = '$estimateCode'&& a.parent='SUB' && a.p_code not in ('SPICKUP003','SSEND007') order by a.stDate,pos asc";
									//echo $qryr;
						...
						?>
											...
											<td colspan="4">
												<select class="form-control" name="mtarea[]">
											...
											<td colspan="3" class="no-right-border">
												<div class="row">
													<div class="col-sm-12">
														<!-- <input type="date" id="" name="startDate" class="inpubase" value=""/> -->
														<div class="input-group input-group-sm">
															<input type="text" name="singleDayTourStartDate[]" class="form-control js-singleDayTourDate js-singleDayTourDate1" aria-label="출발일" placeholder="출발일" value='<?=$local_start?>' readonly>
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						<tr>
							<td colspan="16" class="active text-center formHeader fullWidth">항공정보 &nbsp;<button type="button" class="btn btn-default btn-xs js-hideShowToggle"><span class="glyphicon glyphicon-chevron-down" aria-hidden="true"></span></button></td>
						</tr>
						<tr>
							<td colspan="2" class="active text-center formHeader">도착정보</td>
							<td colspan="2">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											...
											<input type="text" name="arrivecity" class="form-control " aria-label="출발도시" value="<?=$reserve_info[air_stcity]?>"/>
										</div>
									</div>
								</div>
							</td>
							...
							<td colspan="2">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="arrivalTime" class="form-control js-timeInput js-arrivalTime" aria-label="도착시간" placeholder="도착시간" value="<?=$reserve_info[air_arrivetime]?>">
											<span class="input-group-btn">
						...
						<tr>
							<td colspan="2" class="active text-center formHeader">출발정보</td>
							<td colspan="2"></td>
							<td colspan="2">
							...
							<td colspan="2">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="input-group input-group-sm">
											<input type="text" name="departureTime" class="form-control js-timeInput js-departureTime" aria-label="출발시간" placeholder="출발시간" value="<?=$reserve_info[air_sttime]?>">
											<span class="input-group-btn">
												<button class="btn btn-default js-timeInputBtn" type="button"><span class="glyphicon glyphicon-time" aria-hidden="true"></span></button>
											</span>
										</div>
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						<tr>
							<td colspan="16" class="active text-center formHeader fullWidth">연계예약 &nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default btn-xs js-addtour" <?php if (!$estimateCode) {?>disabled  <?php } ?> >투어추가 <span class="glyphicon glyphicon-plus" aria-hidden="true" ></span></button> &nbsp;<button type="button" class="btn btn-default btn-xs js-addhotel" <?php if (!$estimateCode) {?> disabled  <?php } ?>>호텔추가 <span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button></td>
						</tr>
					    <?php
				...
				<table class="table table-bordered table-condensed gridSixteen reserveTable formDetail">
					<tbody>
						...
						<tr>
							...
							<td colspan="6"><?=$prodInfo[base_rate]?></td>
							<td colspan="2" class="active text-center formHeader">적용환율</td>
							<td colspan="3">기준환율-실시간(BUY) : <?=$Brate?></td>
							<td colspan="3">기준환율-실시간(SELL) : <?=$Srate?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3" class="no-right-border"><input type="text" class="form-control" name="paymemo" value="<?=$reserve_info[pay_memo]?>"></td>
							<td colspan="3" class="no-left-border">
								<div class="row">
									<div class="col-sm-6">
										<button type="button" class="btn btn-xs btn-block btn-default js-setSecurity" <?php if ((!$estimateCode) || ($reserve_info[payment_st] == "GPAY")) {?> disabled  <?php } ?> onClick="go_guarantee()">보증</button>
									</div>
									<div class="col-sm-6">
										<button type="button" class="btn btn-xs btn-block btn-default js-cancelSecurity" <?php if (!$estimateCode) {?>disabled  <?php } ?> onClick="go_cguarantee()">보증취소</button>
									</div>
								</div>
							</td>
						</tr>
				...
				<div class="row">
					<div class="col-sm-6 col-sm-offset-6 text-right">
						<?php if ($estimateCode) { ?>
							<button type="button" class="btn btn-xs btn-default js-rr" onClick="go_submit()">예약수정저장</button>
							<button type="button" <?php if (((($reserve_info[rev_status]=='ORDER') || ($reserve_info[rev_status]=='CANCEL') ||  ($reserve_info[rev_status]=='DONE')) && ($reserve_info[rev_status]!='')) || ($revst == 'WAIT')){ ?> class="btn btn-xs btn-default js-cr disabled" disabled <? } else { ?> class="btn btn-xs btn-default js-cr" <?php } ?> onClick="go_order()">예약확정</button>
							<button type="button" <?php if (((($reserve_info[rev_status]=='DONE') || ($reserve_info[rev_status]=='CANCEL') ||
							($reserve_info[rev_status]=='READY')) && ($reserve_info[rev_status]!='')) || ($revst == 'WAIT')) { ?> class="btn btn-xs btn-default js-done disabled" disabled <? } else { ?> class="btn btn-xs btn-default js-ccr" <?php } ?> onClick="go_corder()">최종안내</button>
							<button type="button" class="btn btn-xs btn-default js-can" onClick="go_cancel()">예약취소</button>
							<button type="button" class="btn btn-xs btn-default js-prn" onClick="javascript:openwin('<?=$estimateCode?>')">고객 영수증 출력</button>
					  <?php } else { ?>
								...
								<div class="row">
									<div class="col-sm-12">
										...
										<table class="table table-bordered table-condensed gridSixteen paymentProcessingTable formDetail">
											<tbody>
												...
												<tr>
													...
													<td colspan="2" class="active text-center formHeader">환율적용금액</td>
													<td colspan="1" class="active text-center formHeader">환율적용</td>
												...
												<?php
												 ...
												 ?>
																		<tr>
																			...
																			<td colspan="1" class="text-center"><?=$rate_m?></td>
												 ...
												 ?>
																		<tr>
																			...
																			<td colspan="1" class="text-center"><?=$rate_c?> <?=$rate_m?></td>
																			<td colspan="1" class="text-center"><?=$p_row[payment_status]?></td>
																			<td colspan="1" class="text-center"><?=$uinfo[kor_name]?></td>
												...
												<tr>
													...
													<td colspan="4" class="no-right-border">
														<select class="form-control js-paymentType" name="paymentmethod" id="paymentmethod">
															...
															<option value="debitcard">데빗</option>
															<option value="check">체크</option>
															<option value="banktransfer">은행송금</option>
															<option value="creditcard">신용카드웹</option>
															<option value="bcreditcard">신용카드 자사단말기</option>
															
															<option value="giftcertificate">파란여행상품권</option>
										...
										<table class="table table-bordered table-condensed gridSixteen paymentProcessingTable formDetail hidden js-paymentCrediCard">
											<tbody>
												<tr>
													...
													<td colspan="4" class="no-right-border">
														<input type="text" name="cardname" class="form-control" placeholder="카드소유주-영문">
													</td>
													<td colspan="10" class="no-left-border"></td>
												</tr>
												<tr>
												...
												<tr>
													<td colspan="2" class="active text-center formHeader">적용환율</td>
													<td colspan="2" class="no-right-border">
														<input type="text" class="form-control buyrate1" name='buyrate1' value="" placeholder="BUY">
													</td>
													<td colspan="1" class="active text-center formHeader"><button type="button" class="btn btn-primary js-rate">환율적용</button></td>
													
													<td colspan="10" class="no-left-border"></td>
												</tr>
												<tr>
													<td colspan="2" class="active text-center formHeader">결제받을금액</td>
													<td colspan="2" class="no-right-border">
														<input type="text" class="form-control" name='lastamt' value="" placeholder="결제받을금액">
													</td>
													<td colspan="2" class="active text-center formHeader">환율결제금액</td>
													<td colspan="2" class="no-right-border">
										...
										<table class="table table-bordered table-condensed gridSixteen paymentProcessingTable formDetail hidden js-paymentOther">
											<tbody>
												<tr>
													<td colspan="2" class="active text-center formHeader">결제통화종류</td>
													<td colspan="6">
														<label class="checkbox-inline">
															<input type="radio" name="currencytype" value="CAD" <?php if ($reserve_info[base_rate] == "CAD") {?> checked <?php }?>> CAD
														</label>
														<label class="checkbox-inline">
															<input type="radio" name="currencytype" value="USD" <?php if ($reserve_info[base_rate] == "USD") {?> checked <?php }?>> USD
														</label>
														<label class="checkbox-inline">
															<button type="button" class="btn btn-primary js-processrate">환율적용</button>
														</label>
													</td>
													<td colspan="2" class="active text-center formHeader">적용환률</td>
													<td colspan="1" class="no-right-border">
														BUY
													</td>
													<td colspan="2" class="no-right-border">
														<input type="text" name="buyrate" class="form-control buyrate" placeholder="BUY" value="">
													</td>
													<td colspan="1" class="no-right-border">
														SELL
													</td>
													<td colspan="2" class="no-left-border">
														<input type="text" name="sellrate" class="form-control sellrate" placeholder="SELL" value="">
													</td>
												</tr>
												<tr>
													<td colspan="2" class="active text-center formHeader">결제금액 <span id="psign"><font color="red"></font></span></td>
													<td colspan="6">
														<input type="text" name="rpay" id="rpay" class="form-control" placeholder="결제금액" value="">
														<input type="hidden" name="opay" id="opay" class="form-control" placeholder="결제금액" value="">
													</td>
													<td colspan="2" class="active text-center formHeader">환율결제금액</td>
													<td colspan="6">
								...
								<div class="row">
									<div class="col-sm-12">
										...
										<table class="table table-bordered table-condensed gridSixteen paymentProcessingTable formDetail js-paymentOther">
											<tbody>
												<tr>
													<td colspan="2" class="active text-center formHeader">환불통화종류</td>
													<td colspan="6">
														<label class="checkbox-inline">
															<input type="radio" name="currencytype2" value="CAD" <?php if ($reserve_info[base_rate] == "CAD") {?> checked <?php }?>> CAD
														</label>
														<label class="checkbox-inline">
															<input type="radio" name="currencytype2" value="USD" <?php if ($reserve_info[base_rate] == "USD") {?> checked <?php }?>> USD
														</label>
														<label class="checkbox-inline">
															<button type="button" class="btn btn-primary js-Rprocessrate">환율적용</button>
														</label>
													</td>
													<td colspan="2" class="active text-center formHeader">적용환률</td>
													<td colspan="1" class="no-right-border">
													...
													<td colspan="2" class="no-right-border">
														<input type="text" name="buyrate2" class="form-control" placeholder="BUY" value="<?=$Brate?>">
													</td>
													<td colspan="1" class="no-right-border">
														SELL
													</td>
													<td colspan="2" class="no-left-border">
														<input type="text" name="sellrate2" class="form-control" placeholder="SELL" value="<?=$Srate?>">
													</td>
												</tr>
												<tr>
													...
													<td colspan="2" class="active text-center formHeader">환불금액</td>
													<td colspan="6">
														<input type="text" name="lastpayamt2" id="lastpayamt2" class="form-control" placeholder="환불금액" value="" >
													</td>
												</tr>
												<tr>
    ...
    <script>
		$(document).ready(function () {
					...
					*/
					...
					tourPricePick.off('click').on('click', function (e) {
						...
						priceOptions.find('a').off('click').on('click', function (e) {
							...
							var pickPriceType = section.find('.js-pickPriceType')
							pickPriceType.val(arr[1])
							var pickRoomType = section.find('.js-pickRoomType')
							pickRoomType.val(arr[0])
							
							var pickPriceType1 = section.find('.js-pickPriceType1')
							pickPriceType1.val(arr[1])
							var pickRoomType1 = section.find('.js-pickRoomType1')
			...
			$('.js-addTraveler').keyup(function() {
				
				//ratecalc();

			});
			$('.js-removeTraveler').keyup(function() {
		...
		function go_Rpay() {
				...
				if ($('input[name^="lastpayamt2"]').val()=="")
				{
					alert("최종환불금액을 입력하세요!");
					$('input[name^="lastpayamt2"]').focus();
					return;
				}
