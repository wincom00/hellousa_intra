    ...
    function printProduct() {
	   ...
	   //echo $tourCategory;
		if ($pname) {
			...
			foreach($keywords as $kw) {
				  ...
				  $kw = $dbConn->real_escape_string($kw);
				  $likes[] = "p_name LIKE '%{$kw}%'";
			}
		   // $qrynm = "&& a.p_name like '%$pname%'";
		   $qrynm =" && " .implode(" AND ", $likes)." || a.p_code like '%$pname%'";
		   //echo $qrynm;
		   //exit;
	    } else {
			$qrynm = "";
	   ...
	   //$startDate = "9" - 매일출발;
		...
		while($row1 = $rst1->fetch_assoc()){
			...
			if (($row1[p_own] == "dongbu") || ($row1[p_own] == "hello")){
				$randrow[kor_name] = "TOUR헬로USA";
			} else {
									...
									<div class="row">
										<div class="col-sm-6">
											<select class="form-control fst1" name="area1" id="area1">
												<option value="">- 선택 -
												<?=printBaseCode_first('P01',$area1)?>
											</select>
										</div>
										<div class="col-sm-6">
											<select class="form-control fst2" name="area2" id="area2">
												<option value="">- 선택 - 
												<?=printBaseCode_second('P01','',$area2)?>
											</select>
										</div>
									</div>
								</td>
