...
//exit;
	...
	$rst1 = $dbConn->query($qry1);
	$result_rows= $rst1->num_rows;

	$total=$mysqli -> affected_rows;
	$last=floor($total/$scale);
	
	$page_total_qry = $dbConn -> query("select count(*) 
									from att_log a,member_list b
									where a.userid=b.userid $emp $comp_qry &&
										date_format( a.login_date, '%Y-%m-%d' ) between '$st' and '$ed'");
	...
	function printattlog() {
		global $dbConn, $division, $mCode, $mode, $st, $ed, $start, $page_scale, $scale, $rst1, $qry1, $page_total, $user_dbinfo;
//echo $qry1;
		if ($mode <> "modify") {
			if ($page_total != "0") {
			...
			//echo $row1[lodate];
				...
				*/
				...
				if ($user_dbinfo[userid] == "ethandoh") {
												...
												<select name=empid class="form-control">
													...
													<? 
														$que = "select *
																...
																order by kor_name";
														$que_rst1 =$dbConn->query($que);
																		
														while ($que_row1 = $que_rst1->fetch_assoc()): 
						...
						<script>
							function del(id){
								if (confirm("삭제할까요?") == true) {
									location.replace('att_reg.php?division=<?=$division?>&mCode=<?=$mCode?>&st=<?=$st?>&ed=<?=$ed?>&empid=<?=$empid?>&company_code=<?=$company_code?>&mode=del&id=' + id);
								} else return;
							}
						</script>
						  ...
						  <br><br>
						  <p align=center><? pageNavigation(); ?></p>
               </div>
          </div>
      </div>
</div>
