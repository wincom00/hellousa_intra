...
<?php
	...
	function custlist() {
		 global $dbConn,$s_date1,$s_date2,$_POST,$rand_id,$table_content1,$totpmt,$totpmt1,$totcnt,$table_content2;
		 extract($_POST);
		 $sum1 = 0;
		 $tot1 = 0;
		 for($i=0; $i<count($_POST[seqNo]); $i++)
		 {
			...
			while($row1 = $rst1->fetch_assoc()){
				...
				$qry2 = "select *
							from 
								rand_pay
							where rand_id = '$rand_id' && seq_rand='$row1[seqr]'  && trans_type='debit'";
				//print_r($qry2);
		...
		$table_content2 = "<table id='level4' class='table table-striped table-bordered table-hover table-condensed text-center'>
			<th>
				
				<td width=20% align=center>업체 크레딧</td>
				<td width=13% align=center>$".$sum1."</td>
	...
	if ($mode == "send_email") {
			...
			$content = file_get_contents( 
					"invoice_cc.php?rand_id=$rand_id&s_date1=$s_date1&s_date2=$s_date2", false, $stream); 
			  
			echo $rand_id.$s_date1.$s_date2.$seqNo."<br />";
			//print_r($data);
			echo "invoice_cc.php?rand_id=$rand_id&s_date1=$s_date1&s_date2=$s_date2";
			exit;
            // 메세지
			$board_pds_pos = "/var/www/vhosts/dongbutour.online/httpdocs/upload";
			...
			echo $content."TETS";
			exit;
			$ret= mailsend_m($cmail,$sbj,$msg,$attachment1,$attachment2);
			//echo $cmail;
			//exit;
			$ret= mailsend_m('admin@dongbutour.com',$sbj,$msg,$attachment1,$attachment2);
				...
				<div class="col-sm-12 col-md-12">
						...
						<div class="col-sm-12">
						    <form action="<?php $PHP_SELF;?>" name="frmName" id="frmName" method="post">
							  ...
							  <input type="hidden" name="imail" value="<?=$_GET[iemail]?>">
                                <? if (($mode != 'down') || ($mode != 'print')) { ?>
										...
										<br/>
											<table class="table table-bordered table-condensed">
													...
													<tr>
														...
														<td width="40%" class=""><?=$g_dbinfo[kor_name]?>(<?=$g_dbinfo[userid]?>)
															
														</td>
														
													</tr>
													<tr>
														<td width="10%" class="titletd text-center">주소/연락처</td>
														<td width="40%" class=""><?=$g_dbinfo[address]?>(<?=$g_dbinfo[company_phone]?>)
