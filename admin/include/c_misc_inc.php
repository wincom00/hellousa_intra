 ...
 function setCookie($name, $value = '', $expire = 0, $path = '', $domain= '', $secure = 0) {
	...
	*/
	...
	function checkEmail($email) {
		if (!preg_match('/^[^@ ]+@[a-zA-Z0-9\-]+\.+[a-zA-Z0-9\-]/i', $email)) {
			return false;
  		}
  		/* 한글이 포함되었는지 체크 */
	...
	*/
	...
	function fileDownload($fileDir='.', $fileName, $realFileName='') {
	    ...
	    $ie_v=preg_replace('/^.+MSIE /', '', $HTTP_USER_AGENT);
    	$ie_v=strtok($ie_v,';');
	...
	// $attachFileName : 업로드시 파일이름.
 	...
 	static function uploadFileUnsafely($attachFile, $attachFileName, $saveDir = '.'){
   		...
   		//$saveDir = preg_replace('/\/$/', '', $saveDir);
		$pattern = '/$/i'; 
		$saveDir = preg_replace($pattern, '', $saveDir);
  	...
  	// 파일업로드 확장자 바꿈
 	function uploadFileUnsafely_change($attachFile, $attachFileName, $saveDir = '.'){
 	...
 	function checkUrl($url){
    	if (!preg_match('/[a-zA-Z0-9\-\.]+\.[a-zA-Z0-9\-\.]+.*/i', $url)) {
			return;
    	}
