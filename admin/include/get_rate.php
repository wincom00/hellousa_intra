...
function fred_latest($seriesId) {
    ...
    // 유효성 검사
    if (!isset($obs['rate']) || $obs['rate'] === '.' || $obs['rate'] === '') {
        return null;
    }

    // ✅ realtime_start, date, value 모두 반환
    return [
        'date' => isset($obs['updated_at']) ? $obs['updated_at'] : '',
...
function get_rate_via_fred($code) {
    ...
    if ($type === 'PER_USD') {
        $x_per_usd = (float)$x['rate'];
    } else { // USD_PER
        $usd_per_x = (float)$x['rate'];
    ...
    $updated   = ($krw['date'] > $x['updated_at']) ? $krw['date'] : $x['updated_at'];
