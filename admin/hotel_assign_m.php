  ...
  /* if (!hasMenuAccess($division, $pdx, $sub)) {
	...
	*/
	...
	function buslist() {
       ...
       //echo $qry1;
		...
		while($row1 = mysqli_Fetch_assoc($rst1)){
			$s = $k+1;
			//$rcnt = getHRoomCnt($row1[grand_eCode],$row1[sub_eCode]);
									...
									<div class='col-sm-3'>
										<div class='input-group input-group-sm'>       
										   ...
										   if ($gscode == $row1[sub_eCode]) {
										         echo "<input type='radio' name='roomnumber' value='$s' checked onClick='selectcar(\"$row1[sub_eCode]\")'>방갯수 : $rcnt[rcnt1] 개";
										   } else {
											   echo "<input type='radio' name='roomnumber' value='$s' onClick='selectcar(\"$row1[sub_eCode]\")' >방갯수 : $rcnt[rcnt1] 개";
								...
								<div class='col-sm-12'>
									...
									<div class='col-sm-3'>
										<div class='input-group input-group-sm'>       
										   <label class='radio-inline'>";
											if ($gscode == $row1[sub_eCode]) {
										         echo "<input type='radio' name='roomnumber' value='$s' onClick='selectcar(\"$row1[sub_eCode]\")' checked >방갯수 : $rcnt[rcnt1] 개";
										   } else {
											   echo "<input type='radio' name='roomnumber' value='$s'onClick='selectcar(\"$row1[sub_eCode]\")' >방갯수 : $rcnt[rcnt1] 개";
	...
	function Hotellist() {
		...
		//echo $pcode;
		if(strstr($pcode, "ADD") !== "") {  
			$qry3 = " SELECT p_day  as days FROM product_master where p_code = '$pcode'";
		   ...
		   // echo $qry1;
			...
			while($row1 = mysqli_Fetch_assoc($rst1)){
			 ...
			 $qry2 ="select seq_no,grand_eCode, sub_eCode, hotel_code,p_code,p_name,pcnt,day from hotel_assign
			...
			//echo $qry2;
			 ...
			 while($row2 = mysqli_Fetch_assoc($rst2)){
					...
					$startDate = new DateTime($st); // 시작 날짜/시간으로 DateTime 객체를 생성합니다.
					$daysToAdd = (int) $row1['day']-1; // 더할 날짜 수를 정수로 변환합니다.

					$interval = new DateInterval("P{$daysToAdd}D"); // DateInterval 객체를 생성합니다.
					$startDate->add($interval); // 시작 날짜에 시간 간격을 더합니다.
					$stDate_sub = $startDate->format("Y-m-d"); // 결과를 Y-m-d 형식으로 포맷하여 stDate_sub에 저장합니다.
					...
					echo"<tr>
							...
							<td>$hinfo[h_name]</td>
							<td><input type='number' name='pcnt[]' class='form-control'  min=1 value='$row2[pcnt]'></td>
							<input type='hidden' name='day[]'  value='{$row2['day']}'>
							<input type='hidden' name='ssub[]'  value='".$stDate_sub."'>
							<td><button type=button class='form-control btnup' placeholder='업데이트' value=".$row2['seq_no'].">업데이트</button>|<button type=button class='form-control btndel' placeholder='삭제' value=".$row2['seq_no'].">삭제</button></td>
						</tr>";
					$j++;  
			...
			$qry2 ="select grand_eCode, sub_eCode, hotel_code,p_code,p_name,day from hotel_assign
...
?>
    ...
    <script>
		...
		$('.btnup').on('click', function() {
			// Find the parent table row (<tr>) of the clicked button
			var $row = $(this).closest('tr');

			// Get the sequence number from the button's value
			var seq_no = $(this).val();

			// Find and get the values from the input fields within that specific row
			var sub_eCode = $row.find('input[name="sub[]"]').val();
			var pcnt = $row.find('input[name="pcnt[]"]').val();
			var day = $row.find('input[name="day[]"]').val();
			var ssub = $row.find('input[name="ssub[]"]').val();

			// --- Optional: Basic Client-Side Validation ---
			if (!pcnt || pcnt < 1) {
				alert('수량(pcnt)은 1 이상이어야 합니다.'); // "Quantity (pcnt) must be 1 or greater."
				$row.find('input[name="pcnt[]"]').focus();
				return; // Stop the process
			}
			// Add other validation if needed

			// Send the data to the PHP script using AJAX (POST method is common for updates)
			$.ajax({
				url: 'update_cnt.php', // <<<--- IMPORTANT: Set the path to your PHP file
				type: 'POST',
				data: {
					seq_no: seq_no,
					sub_eCode: sub_eCode,
				...
				dataType: 'json', // Expect a JSON response from PHP
				success: function(response) {
					// Handle the response from PHP
					if (response.status === 'success') {
						alert(response.message); // Show success message (e.g., "업데이트 성공!")
						// Optional: Maybe visually indicate success on the row
						$row.css('background-color', '#d4edda'); // Example: highlight green
						setTimeout(function() {
							 $row.css('background-color', ''); // Remove highlight after a delay
						}, 2000);
					} else {
						alert('업데이트 실패: ' + response.message); // Show error message
						// Optional: Highlight row in red or provide more specific feedback
						 $row.css('background-color', '#f8d7da'); // Example: highlight red
					}
				},
				error: function(jqXHR, textStatus, errorThrown) {
					// Handle AJAX errors (e.g., network issue, server error)
					alert('서버 통신 오류가 발생했습니다: ' + textStatus); // "Server communication error occurred"
