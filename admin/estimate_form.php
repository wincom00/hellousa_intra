...
/* ====== 초기값 (필요 시 DB에서 로드하도록 교체) ====== */
$today  = date('Y-m-d');
$id     = (int)($_GET['id'] ?? 0);
$master = null; $items = [];

if ($id) {
  $m = $dbConn->query("SELECT * FROM estimate_master WHERE id=".$id);
  if ($m && $m->num_rows) $master = $m->fetch_assoc();
  $q = $dbConn->query("SELECT * FROM estimate_items WHERE estimate_id=".$id." ORDER BY section,id");
  if ($q) $items = $q->fetch_all(MYSQLI_ASSOC);
}
...
<script>
/* 서버 데이터 → JS로 전달 */
window.EST_DATA = <?=json_encode(['master'=>$master,'items'=>$items], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;
window.HOTEL_OPTIONS_HTML    = '<?= str_replace(["\n","\r","'"], ["","","\\'"], $hotelOptionsHtml) ?>';
window.ENTRANCE_OPTIONS_HTML = '<?= str_replace(["\n","\r","'"], ["","","\\'"], $entranceOptionsHtml) ?>';
</script>
