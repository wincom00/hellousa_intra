 ...
 * - OVERTIME: 날짜별 시간/단위 합산 → 합계 = SUM(날짜열)*대상수(unit_per_target)
...
if (isset($_GET['action']) && $_GET['action'] === 'download_excel') {
    ...
    <form method='GET'>
      ...
      <input type='number' id='id' name='id' required placeholder='estimate_master의 id'></div>
...
function streamBreakdownExcelXlsxStyled(mysqli $dbConn, int $estimateId) {
    ...
    // dates(맵/리스트) → "일수/시간" 맵으로 변환 (리스트면 각 날짜 1)
    ...
    foreach (range(1,$MAX_COL_IDX) as $i) {
        $c = $colByIdx($i);
        $sheet->getColumnDimension($c)->setWidth(in_array($c,['A','B','C','D','H','I'])?14:10);
    }
    ...
    $order=['HOTEL','MEAL','TRANSPORT','OVERTIME','TICKET','GUIDE','ETC']; // ← OVERTIME 추가
    $titles=[
        'HOTEL'=>'1) HOTEL',
        'MEAL'=>'2) MEAL (일자×식사 매트릭스)',
        'TRANSPORT'=>'3) TRANSPORTATION (일자별 차량료)',
        'OVERTIME'=>'3-1) OVERTIME',
        'TICKET'=>'4) 입장권',
        'GUIDE'=>'5) 가이드/기사',
        'ETC'=>'7) 기타경비'
    ];
    ...
    // OVERTIME 헤더용 날짜
    $overtimeAllDates = $mealAllDates;
    if (empty($overtimeAllDates) && !empty($sections['OVERTIME'])) {
        $set = [];
        foreach ($sections['OVERTIME'] as $it) {
            $etc = json_decode((string)($it['etc_json']??'{}'), true) ?: [];
            $m = $datesToDaysMap($etc['dates'] ?? []);
            if (empty($m)) {
                $d0 = $resolveDate('OVERTIME',$etc) ?: date('Y-m-d');
                $m = [$d0 => (float)($it['qty'] ?? 1)];
            }
            foreach ($m as $ymd => $_) $set[$ymd] = true;
        }
        $overtimeAllDates = array_keys($set); sort($overtimeAllDates);
        if (empty($overtimeAllDates)) $overtimeAllDates = [date('Y-m-d')];
    }
    ...
    if (count($overtimeAllDates) > $TRANS_MAX_DATES) $overtimeAllDates = array_slice($overtimeAllDates, 0, $TRANS_MAX_DATES);
    ...
    $OVERT_LAST_COL_IDX  = !empty($overtimeAllDates)? (1 + count($overtimeAllDates)+ 2) : 0;
    $LAST_COL_IDX = min($MAX_COL_IDX, max(8, $MEAL_LAST_COL_IDX, $TRANS_LAST_COL_IDX, $OVERT_LAST_COL_IDX));
    $L = $colByIdx($LAST_COL_IDX);

    // 합계/우측 영역 병합 헬퍼
    $mergeToSheetEnd = function($row, $fromColLetter) use ($sheet, $L) {
        if ($L !== $fromColLetter) $sheet->mergeCells("{$fromColLetter}{$row}:{$L}{$row}");
    };
    ...
    $setFill("A{$row}:{$L}{$row}", $CLR_BLUE);
    $setFontColor("A{$row}:{$L}{$row}", $CLR_WHITE);
    $row += 2;

    // === 기본정보 ===
    $firstInfoRow = $row;
    $infoRow1 = ['TO',(string)($master['to_name']??''),'GROUP',(string)($master['group_name']??''),'PAX',(int)($master['pax']??0),'FOC',(int)($master['foc']??0)];
    $infoRow2 = ['시작일',(string)($master['start_date']??''),'종료일',(string)($master['end_date']??''),'총인원',(int)($master['total_pax']??0),'작성일',(string)($master['wdate']??'')];

    $sheet->fromArray([$infoRow1], null, "A{$row}");
    $sheet->fromArray([$infoRow2], null, "A".($row+1));

    foreach (['A','C','E','G'] as $c){ $setFill("{$c}{$row}", $CLR_HGRAY); $setFill("{$c}".($row+1), $CLR_HGRAY); }

    $lastValCol1 = \PHPExcel_Cell::stringFromColumnIndex(count($infoRow1) - 1);
    $lastValCol2 = \PHPExcel_Cell::stringFromColumnIndex(count($infoRow2) - 1);
    $sheet->mergeCells("{$lastValCol1}{$row}:{$L}{$row}");
    $sheet->mergeCells("{$lastValCol2}".($row+1).":{$L}".($row+1));

    $sheet->getStyle("A{$row}:{$L}".($row+1))->applyFromArray($borderAll + $vbCenter);
    $sheet->getStyle("{$lastValCol1}{$row}:{$L}{$row}")->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT)->setWrapText(true);
    $sheet->getStyle("{$lastValCol2}".($row+1).":{$L}".($row+1))->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT)->setWrapText(true);
    ...
    foreach ($order as $sec) {
        ...
        // OVERTIME (신규)
        if ($sec==='OVERTIME') {
            // 헤더: A=대상 | dates... | 대상수 | 합계
            $col=1; $sheet->setCellValue($colByIdx($col).$row,'대상'); $col++;
            foreach($overtimeAllDates as $ymd){ $sheet->setCellValue($colByIdx($col).$row,$ymd); $col++; }
            $colCnt=$col;       $sheet->setCellValue($colByIdx($col).$row,'대상수'); $col++;
            $colTotal=$col;     $sheet->setCellValue($colByIdx($col).$row,'합계');
            $mergeToSheetEnd($row, $colByIdx($colTotal));
            ...
            $rows = []; // key => ['title'=>..., 'cnt'=>N, 'days'=>[ymd=>hours]]
            foreach ($sections[$sec] as $it) {
                $etc  = json_decode((string)($it['etc_json']??'{}'),true)?:[];
                $target = (string)($etc['target'] ?? ($it['label'] ?? 'Overtime'));
                $key = trim($target);

                if (!isset($rows[$key])) $rows[$key] = ['title'=>$key,'cnt'=>0.0,'days'=>[]];

                // 대상수 누적 (unit_per_target 사용)
                $rows[$key]['cnt'] += (float)($etc['unit_per_target'] ?? (float)($it['cnt'] ?? 0));
                ...
                foreach ($daysMap as $ymd=>$v) {
                    if (!isset($rows[$key]['days'][$ymd])) $rows[$key]['days'][$ymd]=0.0;
                    $rows[$key]['days'][$ymd] += (float)$v;
                }
            }
            ...
            foreach ($rows as $r) {
                ...
                $col=2; $sumCells=[];
                foreach ($overtimeAllDates as $ymd){
                    $addr=$colByIdx($col).$row;
                    $val = isset($r['days'][$ymd]) ? (float)$r['days'][$ymd] : 0.0;
                    $sheet->setCellValueExplicit($addr,$val, PHPExcel_Cell_DataType::TYPE_NUMERIC);
                    $sumCells[]=$addr; $col++;
                }

                $addrCnt=$colByIdx($col).$row;
                $sheet->setCellValueExplicit($addrCnt,(float)$r['cnt'] ?: 1.0, PHPExcel_Cell_DataType::TYPE_NUMERIC);
                $col++;

                // 합계 = SUM(날짜열) * 대상수  (서버 계산식과 동일)
                $addrTot  = $colByIdx($col).$row;
                $sumExpr  = !empty($sumCells)?'('.implode('+',$sumCells).')': '0';
                $sheet->setCellValue($addrTot, "={$sumExpr}*{$addrCnt}");
                $sheet->getStyle($addrTot)->getNumberFormat()->setFormatCode('#,##0.00;-#,##0.00;0');
                $ovRowTotals[]=$addrTot;
            ...
            $sumExpr = !empty($ovRowTotals)?'='.implode('+',$ovRowTotals):'0';
            $sheet->mergeCells("A{$row}:".$colByIdx($colTotal-1)."{$row}");
