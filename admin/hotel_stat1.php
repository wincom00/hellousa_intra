...
/* ============================================================
 * 호텔 정산 통계 (PHP 7.0)
 * - 집계 기준: hotel_settle / hotel_settleetc + tour_guide.stDate
...
$where_assign="WHERE ha.stDate BETWEEN '$esc_from' AND '$esc_to'";
if($pcode) $where_assign.=" AND ha.p_code='".$dbConn->real_escape_string($pcode)."'" ;

$where_tg="WHERE tg.stDate BETWEEN '$esc_from' AND '$esc_to'";
if($pcode) $where_tg.=" AND tg.p_code='".$dbConn->real_escape_string($pcode)."'" ;
...
/* --------------------------- 주간(ISO Week) ---------------------------- */
$SQL_WEEK_ISO = "YEARWEEK(stat_date, 3)"; // ISO 주차(월요일 시작): 예 202601

/* --------------------------- 호텔 통계 데이터 소스 ---------------------------- */
$amountSourceLabel = '호텔비(hotel_settle.hotel_amt) + 기타비(hotel_settleetc.etc_amt)';
$SQL_STAT_BASE = "
(
    SELECT
        ha.stDate AS stat_date,
        ha.hotel_code AS hotel_code,
        MAX(ph.h_name) AS hotel_name,
        ($SQL_PTYPE_NORM) AS region_code,
        SUM(COALESCE(ha.pcnt,0)) AS assign_rooms,
        0 AS settle_rooms,
        0 AS amount
    FROM hotel_assign ha
    LEFT JOIN product_hotel ph ON ph.h_code=ha.hotel_code
    $where_assign
    GROUP BY ha.stDate, ha.hotel_code, ph.p_types

    UNION ALL

    SELECT
        tg.stDate AS stat_date,
        hs.h_code AS hotel_code,
        MAX(ph.h_name) AS hotel_name,
        ($SQL_PTYPE_NORM) AS region_code,
        0 AS assign_rooms,
        SUM(COALESCE(hs.room_cnt,0)) AS settle_rooms,
        SUM(COALESCE(hs.hotel_amt,0)) AS amount
    FROM hotel_settle hs
    JOIN tour_guide tg
      ON tg.grand_eCode=hs.grand_eCode
     AND tg.sub_eCode=hs.sub_eCode
    LEFT JOIN product_hotel ph ON ph.h_code=hs.h_code
    $where_tg
    GROUP BY tg.stDate, hs.h_code, ph.p_types

    UNION ALL

    SELECT
        tg.stDate AS stat_date,
        he.h_code AS hotel_code,
        MAX(ph.h_name) AS hotel_name,
        ($SQL_PTYPE_NORM) AS region_code,
        0 AS assign_rooms,
        0 AS settle_rooms,
        SUM(COALESCE(he.etc_amt,0)) AS amount
    FROM hotel_settleetc he
    JOIN tour_guide tg
      ON tg.grand_eCode=he.grand_eCode
     AND tg.sub_eCode=he.sub_eCode
    LEFT JOIN product_hotel ph ON ph.h_code=he.h_code
    $where_tg
    GROUP BY tg.stDate, he.h_code, ph.p_types
) stat_base
";
...
if(isset($_GET['action']) && $_GET['action']==='export'){
    ...
    switch($type){
        case 'yearly':
            $sql="SELECT DATE_FORMAT(stat_date,'%Y') AS 연도,
                         SUM(assign_rooms) AS 호텔객실수,
                         SUM(settle_rooms) AS 정산객실수,
                         SUM(amount) AS 총금액
                  FROM $SQL_STAT_BASE
                  GROUP BY 연도 ORDER BY 연도 ASC";
            $headers=['연도','호텔객실수','정산객실수','총금액'];
        ...
        case 'weekly':
            $sql="SELECT CONCAT(SUBSTRING($SQL_WEEK_ISO,1,4),'-W',LPAD(SUBSTRING($SQL_WEEK_ISO,5,2),2,'0')) AS 주,
                  ...
                  GROUP BY 주
                  ORDER BY 주 ASC";
            $headers=['주','호텔객실수','정산객실수','총금액'];
            $filename="hotel_weekly_{$from_ym}_{$to_ym}.xlsx";
            break;

        case 'weekly_region':
            $sql="SELECT CONCAT(SUBSTRING($SQL_WEEK_ISO,1,4),'-W',LPAD(SUBSTRING($SQL_WEEK_ISO,5,2),2,'0')) AS 주,
                         COALESCE(region_code,'') AS 지역코드,
                         SUM(assign_rooms) AS 호텔객실수,
                         SUM(settle_rooms) AS 정산객실수,
                         SUM(amount) AS 총금액
                  FROM $SQL_STAT_BASE
                  GROUP BY 주, 지역코드
                  ORDER BY 주 ASC, 총금액 DESC";
            $headers=['주','지역','지역코드','호텔객실수','정산객실수','총금액'];
            $filename="hotel_weekly_region_{$from_ym}_{$to_ym}.xlsx";
            break;

        case 'hotel':
            $sql="SELECT COALESCE(NULLIF(hotel_code,''),'미지정') AS 호텔코드,
                         MAX(COALESCE(NULLIF(hotel_name,''),NULLIF(hotel_code,''),'미지정')) AS 호텔명,
                         SUM(assign_rooms) AS 호텔객실수,
                         SUM(settle_rooms) AS 정산객실수,
                         SUM(amount) AS 총금액
                  FROM $SQL_STAT_BASE
                  GROUP BY COALESCE(NULLIF(hotel_code,''),'미지정')
                  ORDER BY 총금액 DESC, 호텔객실수 DESC, 호텔명 ASC";
            $headers=['호텔코드','호텔명','호텔객실수','정산객실수','총금액'];
            $filename="hotel_by_hotel_{$from_ym}_{$to_ym}.xlsx";
            break;

        case 'hotel_mr':
            $sql="SELECT DATE_FORMAT(stat_date,'%Y-%m') AS 월,
                         COALESCE(region_code,'') AS 지역코드,
                         MAX(COALESCE(NULLIF(hotel_name,''),NULLIF(hotel_code,''),'미지정')) AS 호텔명,
                         COALESCE(NULLIF(hotel_code,''),'미지정') AS 호텔코드,
                         SUM(assign_rooms) AS 호텔객실수,
                         SUM(settle_rooms) AS 정산객실수,
                         SUM(amount) AS 총금액
                  FROM $SQL_STAT_BASE
                  GROUP BY 월, 지역코드, 호텔코드
                  ORDER BY 월 ASC, 총금액 DESC, 호텔객실수 DESC";
            $headers=['월','지역','지역코드','호텔명','호텔코드','호텔객실수','정산객실수','총금액'];
        ...
        case 'monthly_region':
            $sql="SELECT DATE_FORMAT(stat_date,'%Y-%m') AS 월,
                  ...
                  GROUP BY 월, 지역코드
                  ORDER BY 월 ASC, 총금액 DESC, 호텔객실수 DESC";
            $headers=['월','지역','지역코드','호텔객실수','정산객실수','총금액'];
            $filename="hotel_monthly_region_{$from_ym}_{$to_ym}.xlsx";
            break;

        case 'region':
            $sql="SELECT COALESCE(region_code,'') AS 지역코드,
                         SUM(assign_rooms) AS 호텔객실수,
                         SUM(settle_rooms) AS 정산객실수,
                         SUM(amount) AS 총금액
                  FROM $SQL_STAT_BASE
                  GROUP BY 지역코드
                  ORDER BY 총금액 DESC, 호텔객실수 DESC";
            $headers=['지역','지역코드','호텔객실수','정산객실수','총금액'];
    ...
    while($a=$res->fetch_assoc()){
        if($type==='hotel_mr'){
            $label = fmtRegionLabel_from_codebase('', $a['지역코드'], $dbConn);
            $rows[] = [$a['월'], $label, $a['지역코드'], $a['호텔명'], $a['호텔코드'], (int)$a['호텔객실수'], (int)$a['정산객실수'], money_num($a['총금액'])];
            continue;
        }
        if($type==='monthly_region'){
            $label = fmtRegionLabel_from_codebase('', $a['지역코드'], $dbConn);
            $rows[] = [$a['월'], $label, $a['지역코드'], (int)$a['호텔객실수'], (int)$a['정산객실수'], money_num($a['총금액'])];
            continue;
        }
        if($type==='region'){
            $label = fmtRegionLabel_from_codebase('', $a['지역코드'], $dbConn);
            $rows[] = [$label, $a['지역코드'], (int)$a['호텔객실수'], (int)$a['정산객실수'], money_num($a['총금액'])];
            continue;
        }
        if($type==='weekly_region'){
            $label = fmtRegionLabel_from_codebase('', $a['지역코드'], $dbConn);
            $rows[] = [$a['주'], $label, $a['지역코드'], (int)$a['호텔객실수'], (int)$a['정산객실수'], money_num($a['총금액'])];
            continue;
        }
        if(isset($a['호텔객실수'])) $a['호텔객실수']=(int)$a['호텔객실수'];
        if(isset($a['정산객실수'])) $a['정산객실수']=(int)$a['정산객실수'];
        if(isset($a['총금액'])) $a['총금액']=money_num($a['총금액']);
        $rows[]=array_values($a);
    }
...
/* 월별(합계) */
$q_monthly="SELECT DATE_FORMAT(stat_date,'%Y-%m') AS ym,
                   SUM(assign_rooms) AS rooms,
                   SUM(settle_rooms) AS settle_rooms,
...
$q_month_region="
  SELECT DATE_FORMAT(stat_date,'%Y-%m') AS ym,
         COALESCE(region_code,'') AS p_types_norm,
         SUM(assign_rooms) AS rooms,
         SUM(settle_rooms) AS settle_rooms,
         SUM(amount) AS amount
  FROM $SQL_STAT_BASE
  GROUP BY ym, p_types_norm
  ORDER BY ym ASC, amount DESC, rooms DESC";
$month_region_raw=fetch_all_assoc($dbConn->query($q_month_region));

/* 연도별 */
$q_yearly="SELECT DATE_FORMAT(stat_date,'%Y') AS yy,
                  SUM(assign_rooms) AS rooms,
                  SUM(settle_rooms) AS settle_rooms,
                  SUM(amount) AS amount
           FROM $SQL_STAT_BASE
...
/* 호텔별 */
$q_hotel="SELECT COALESCE(NULLIF(hotel_code,''),'미지정') AS hotel_code,
                 MAX(COALESCE(NULLIF(hotel_name,''),NULLIF(hotel_code,''),'미지정')) AS hotel_name,
                 SUM(assign_rooms) AS rooms,
                 SUM(settle_rooms) AS settle_rooms,
                 SUM(amount) AS amount
          FROM $SQL_STAT_BASE
          GROUP BY COALESCE(NULLIF(hotel_code,''),'미지정')
          ORDER BY amount DESC, rooms DESC, hotel_name ASC";
...
$q_hotel_mr="
  SELECT DATE_FORMAT(stat_date,'%Y-%m') AS ym,
         COALESCE(region_code,'') AS p_types_norm,
         MAX(COALESCE(NULLIF(hotel_name,''),NULLIF(hotel_code,''),'미지정')) AS hotel_name,
         COALESCE(NULLIF(hotel_code,''),'미지정') AS hotel_code,
         SUM(assign_rooms) AS rooms,
         SUM(settle_rooms) AS settle_rooms,
         SUM(amount) AS amount
  FROM $SQL_STAT_BASE
  GROUP BY ym, p_types_norm, hotel_code
  ORDER BY ym ASC, amount DESC, rooms DESC";
$hotel_mr_raw=fetch_all_assoc($dbConn->query($q_hotel_mr));

/* ★지역별(전체 기간 합계) - 정규화 코드 사용 */
$q_region_all="
  SELECT COALESCE(region_code,'') AS p_types_norm,
         SUM(assign_rooms) AS rooms,
         SUM(settle_rooms) AS settle_rooms,
         SUM(amount) AS amount
  FROM $SQL_STAT_BASE
  GROUP BY p_types_norm
  ORDER BY amount DESC, rooms DESC";
...
foreach($month_region_raw as $r){
    ...
    $monthRegionMap[$ym][$lab]=($monthRegionMap[$ym][$lab]??0)+(int)$r['rooms'];
}
...
foreach($region_all_raw as $r){
    ...
    $region_all[] = [
        ...
        'code'=>$code,
        'rooms'=>(int)$r['rooms'],
        'settle_rooms'=>(int)($r['settle_rooms'] ?? 0),
        'amount'=>money_num($r['amount'] ?? 0)
    ];
}
    ...
    <form class="form-inline" method="get" action="<?=htmlspecialchars($_SERVER['PHP_SELF'])?>">
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          <span class="small" style="margin-left:10px;">총금액 기준: <?=htmlspecialchars($amountSourceLabel)?></span>
        </div>
      </div>
    </form>
    ...
    <div class="tab-content" style="padding-top:12px;">
      ...
      <div id="tabMonthly" class="tab-pane fade in active">
        ...
        <div class="chart-wrap">
          <div class="chart-title">월별 호텔객실수 (배정 기준)</div>
          <svg id="chartM" class="chart-svg"></svg>
        </div>

        <table class="table table-bordered table-condensed js-sortable">
          <thead><tr><th data-sort-type="date-ym">월</th><th data-sort-type="number">호텔객실수</th><th data-sort-type="number">정산객실수</th><th data-sort-type="number">총금액</th></tr></thead>
          <tbody>
          <?php $sum=0; $sumSettle=0; $sumAmt=0; foreach($monthly as $r): $sum+= (int)$r['rooms']; $sumSettle += (int)$r['settle_rooms']; $sumAmt += money_num($r['amount']); ?>
            <tr>
              <td><?=htmlspecialchars($r['ym'])?></td>
              <td class="text-right"><?=number_format($r['rooms'])?></td>
              <td class="text-right"><?=number_format($r['settle_rooms'])?></td>
              <td class="text-right"><?=number_format($r['amount'])?></td>
            </tr>
          <?php endforeach; ?>
          </tbody>
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sum)?></th><th class="text-right"><?=number_format($sumSettle)?></th><th class="text-right"><?=number_format($sumAmt)?></th></tr></tfoot>
        </table>
        ...
        <div class="panel panel-default" style="margin-top:14px;">
          ...
          <div class="panel-body">
            ...
            <div class="chart-wrap">
              <div class="chart-title" id="chartMRTitle">지역별 호텔객실수</div>
            ...
            <div class="table-responsive" style="margin-top:10px;">
              <table id="tblMonthlyRegion" class="table table-bordered table-condensed js-sortable">
                <thead><tr><th data-sort-type="text">지역</th><th data-sort-type="number">호텔객실수</th></tr></thead>
                <tbody></tbody>
              </table>
            </div>
          </div>
      ...
      <div id="tabWeekly" class="tab-pane fade">
        ...
        <div class="chart-wrap">
          <div class="chart-title">주간별 호텔객실수 (ISO Week)</div>
          <svg id="chartW" class="chart-svg"></svg>
        </div>

        <table class="table table-bordered table-condensed js-sortable">
          <thead><tr><th data-sort-type="text">주</th><th data-sort-type="number">호텔객실수</th><th data-sort-type="number">정산객실수</th><th data-sort-type="number">총금액</th></tr></thead>
          <tbody>
          <?php $sumw=0; $sumwSettle=0; $sumwAmt=0; foreach($weekly as $r): $sumw += (int)$r['rooms']; $sumwSettle += (int)$r['settle_rooms']; $sumwAmt += money_num($r['amount']); ?>
            <tr>
              <td><?=htmlspecialchars($r['wk'])?></td>
              <td class="text-right"><?=number_format($r['rooms'])?></td>
              <td class="text-right"><?=number_format($r['settle_rooms'])?></td>
              <td class="text-right"><?=number_format($r['amount'])?></td>
            </tr>
          ...
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumw)?></th><th class="text-right"><?=number_format($sumwSettle)?></th><th class="text-right"><?=number_format($sumwAmt)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabYearly" class="tab-pane fade">
        ...
        <div class="chart-wrap">
          <div class="chart-title">연도별 호텔객실수</div>
          <svg id="chartY" class="chart-svg"></svg>
        </div>
        <table class="table table-bordered table-condensed js-sortable">
          <thead><tr><th data-sort-type="text">연도</th><th data-sort-type="number">호텔객실수</th><th data-sort-type="number">정산객실수</th><th data-sort-type="number">총금액</th></tr></thead>
          <tbody>
          <?php $sumy=0; $sumySettle=0; $sumyAmt=0; foreach($yearly as $r): $sumy+=(int)$r['rooms']; $sumySettle += (int)$r['settle_rooms']; $sumyAmt += money_num($r['amount']); ?>
            <tr>
              <td><?=htmlspecialchars($r['yy'])?></td>
              <td class="text-right"><?=number_format($r['rooms'])?></td>
              <td class="text-right"><?=number_format($r['settle_rooms'])?></td>
              <td class="text-right"><?=number_format($r['amount'])?></td>
            </tr>
          ...
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumy)?></th><th class="text-right"><?=number_format($sumySettle)?></th><th class="text-right"><?=number_format($sumyAmt)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabHotel" class="tab-pane fade">
        ...
        <div class="chart-wrap">
          <div class="chart-title">호텔별 호텔객실수 (상위 10)</div>
          <svg id="chartH" class="chart-svg"></svg>
        </div>
        <table class="table table-bordered table-condensed js-sortable">
          <thead><tr><th data-sort-type="text">호텔코드</th><th data-sort-type="text">호텔명</th><th data-sort-type="number">호텔객실수</th><th data-sort-type="number">정산객실수</th><th data-sort-type="number">총금액</th></tr></thead>
          <tbody>
          <?php $sumh=0; $sumhSettle=0; $sumhAmt=0; foreach($hotel as $r): $sumh+=(int)$r['rooms']; $sumhSettle += (int)$r['settle_rooms']; $sumhAmt += money_num($r['amount']); ?>
            <tr>
          ...
          <tfoot><tr><th colspan="2">합계</th><th class="text-right"><?=number_format($sumh)?></th><th class="text-right"><?=number_format($sumhSettle)?></th><th class="text-right"><?=number_format($sumhAmt)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabRegion" class="tab-pane fade">
        ...
        <div class="chart-wrap">
          <div class="chart-title">지역별 총 호텔객실수 (상위 10)</div>
          <svg id="chartRegionAll" class="chart-svg"></svg>
        </div>
        <table class="table table-bordered table-condensed js-sortable">
          <thead><tr><th data-sort-type="text">지역 (코드)</th><th data-sort-type="number">호텔객실수</th><th data-sort-type="number">정산객실수</th><th data-sort-type="number">총금액</th></tr></thead>
          <tbody>
          <?php $sumr=0; $sumrSettle=0; $sumrAmt=0; foreach($region_all as $r): $sumr += (int)$r['rooms']; $sumrSettle += (int)$r['settle_rooms']; $sumrAmt += money_num($r['amount']); ?>
            <tr>
              <td>
          ...
          <tfoot><tr><th>합계</th><th class="text-right"><?=number_format($sumr)?></th><th class="text-right"><?=number_format($sumrSettle)?></th><th class="text-right"><?=number_format($sumrAmt)?></th></tr></tfoot>
        </table>
      </div>
      ...
      <div id="tabHotelMR" class="tab-pane fade">
        ...
        <div class="panel panel-default" style="margin-top:12px;">
          <div class="panel-heading">월별 · 지역별 · 호텔별 객실수(배정/정산)</div>
          <div class="panel-body" style="padding:0;">
            <table class="table table-bordered table-condensed js-sortable" style="margin:0;">
              <thead><tr>
                ...
                <th data-sort-type="number">호텔객실수</th>
                <th data-sort-type="number">정산객실수</th>
                <th data-sort-type="number">총금액</th>
              </tr></thead>
              <tbody>
              <?php $sum_mr=0; $sum_mr_settle=0; $sum_mr_amt=0; foreach($hotel_mr as $r): $sum_mr+=(int)$r['rooms']; $sum_mr_settle += (int)$r['settle_rooms']; $sum_mr_amt += money_num($r['amount']); ?>
                <tr>
                  <td><?=htmlspecialchars($r['ym'])?></td>
                  <td>
              ...
              <tfoot><tr><th colspan="4" class="text-right">합계</th><th class="text-right"><?=number_format($sum_mr)?></th><th class="text-right"><?=number_format($sum_mr_settle)?></th><th class="text-right"><?=number_format($sum_mr_amt)?></th></tr></tfoot>
            </table>
          </div>
        </div>
      </div>
...
var M_vals   = <?=json_encode(array_map('intval',array_column($monthly,'rooms')))?>;

var W_labels = <?=json_encode(array_column($weekly,'wk'))?>;
var W_vals   = <?=json_encode(array_map('intval',array_column($weekly,'rooms')))?>;

var Y_labels = <?=json_encode(array_column($yearly,'yy'))?>;
var Y_vals   = <?=json_encode(array_map('intval',array_column($yearly,'rooms')))?>;
...
$hotelTopVals = [];
foreach ($hotelTop as $rr){ $hotelTopVals[] = (int)$rr['rooms']; }
?>
...
function renderMonthlyRegion(ym){
  ...
  document.getElementById('chartMRTitle').textContent = '지역별 호텔객실수 ('+ ym +')';
